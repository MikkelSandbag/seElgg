...
class AzureBlobStorage extends ObjectBehavior {
	...
	public function it_should_read_file($blobProxyFactory, $blobProxy, $getBlobResult) {
		$getBlobResult->getContentStream ()->shouldBeCalled ()->
	...
	public function it_should_return_false_when_cannot_read($blobProxyFactory, $blobProxy) {
		$blobProxy->getBlob ( 'containerName', 'filename' )->shouldBeCalled ()->willThrow ( new ServiceException ( 500 ) );
	...
	public function it_should_return_false_when_cannot_rename($blobProxyFactory, $blobProxy) {
		$blobProxy->copyBlob ( 'containerName', 'filename2', 'containerName', 'filename1' )->shouldBeCalled ()->willThrow ( new ServiceException ( 500 ) );
	...
	public function it_should_write_file($blobProxyFactory, $blobProxy) {
		$blobProxy->createBlockBlob ( 'containerName', 'filename', 'some content', \Mockery::type ( '\WindowsAzure\Blob\Models\CreateBlobOptions' ) )->shouldBeCalled ();
	...
	public function it_should_return_false_when_cannot_write($blobProxyFactory, $blobProxy) {
		$blobProxy->createBlockBlob ( 'containerName', 'filename', 'some content', \Mockery::type ( '\WindowsAzure\Blob\Models\CreateBlobOptions' ) )->willThrow ( new ServiceException ( 500 ) );
	...
	public function it_should_check_if_file_exists($blobProxyFactory, $blobProxy, $getBlobResult) {
		...
		$blobProxy->getBlob ( 'containerName', 'filename' )->shouldBeCalled ()->willThrow ( new ServiceException ( 404 ) );
		
		$this->exists ( 'filename' )->shouldReturn ( false );
		
		$blobProxy->getBlob ( 'containerName', 'filename2' )->shouldBeCalled ()->willReturn ( $getBlobResult );
	...
	public function it_should_not_mask_exception_when_check_if_file_exists($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->getBlob ( 'containerName', 'filename' )->shouldBeCalled ()->willThrow ( new \RuntimeException ( 'exists' ) );
	...
	public function it_should_get_file_mtime($blobProxyFactory, $blobProxy, $getBlobPropertiesResult, $blobProperties) {
		...
		$blobProxy->getBlobProperties ( 'containerName', 'filename' )->shouldBeCalled ()->willReturn ( $getBlobPropertiesResult );
		
		$getBlobPropertiesResult->getProperties ()->shouldBeCalled ()->willReturn ( $blobProperties );
		
		$blobProperties->getLastModified ()->shouldBeCalled ()->willReturn ( new \DateTime ( '1987-12-28 20:00:00' ) );
	...
	public function it_should_return_false_when_cannot_mtime($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->getBlobProperties ( 'containerName', 'filename' )->shouldBeCalled ()->willThrow ( new ServiceException ( 500 ) );
	...
	public function it_should_not_mask_exception_when_get_mtime($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->getBlobProperties ( 'containerName', 'filename' )->shouldBeCalled ()->willThrow ( new \RuntimeException ( 'mtime' ) );
	...
	public function it_should_delete_file($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->deleteBlob ( 'containerName', 'filename' )->shouldBeCalled ();
	...
	public function it_should_return_false_when_cannot_delete_file($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->deleteBlob ( 'containerName', 'filename' )->shouldBeCalled ()->willThrow ( new ServiceException ( 500 ) );
	...
	public function it_should_not_mask_exception_when_delete($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->deleteBlob ( 'containerName', 'filename' )->shouldBeCalled ()->willThrow ( new \RuntimeException ( 'delete' ) );
	...
	public function it_should_not_mask_exception_when_get_keys($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->listBlobs ( 'containerName' )->shouldBeCalled ()->willThrow ( new \RuntimeException ( 'keys' ) );
	...
	public function it_should_handle_dirs($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->getBlob ( 'containerName', 'filename' )->shouldNotBeCalled ();
		$blobProxy->getBlob ( 'containerName', 'filename/' )->shouldBeCalled ()->willThrow ( new ServiceException ( 404 ) );
		$blobProxy->getBlob ( 'containerName', 'dirname/' )->shouldBeCalled ();
	...
	public function it_should_create_container($blobProxyFactory, $blobProxy) {
		...
		$blobProxy->createContainer ( 'containerName', null )->shouldBeCalled ();
