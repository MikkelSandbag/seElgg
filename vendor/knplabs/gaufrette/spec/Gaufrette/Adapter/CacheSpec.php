...
class CacheSpec extends ObjectBehavior {
	...
	function it_handles_metadata_when_cached_adapters_supports_metadata($extendedSource, $extendedCache) {
		$extendedSource->setMetadata ( 'filename', array (
				'metadata' 
		) )->shouldBeCalled ();
		$extendedCache->setMetadata ( 'filename', array (
				'metadata' 
		) )->shouldBeCalled ();
		$extendedSource->getMetadata ( 'filename' )->shouldBeCalled ()->willReturn ( array (
				'someMetadata' 
		) );
...
interface CacheTestExtendedAdapter extends \Gaufrette\Adapter, \Gaufrette\Adapter\ChecksumCalculator, \Gaufrette\Adapter\MetadataSupporter {
}
