...
class DropboxSpec extends ObjectBehavior {
	...
	function it_checks_if_file_exists($dropbox) {
		$dropbox->getMetaData ( 'filename', false )->willReturn ( array (
				"size" => "225.4KB",
				"rev" => "35e97029684fe",
				"thumb_exists" => false,
				"bytes" => 230783,
		...
		$dropbox->getMetaData ( 'filename', false )->willThrow ( new \Dropbox_Exception_NotFound () );
	...
	function it_does_not_mask_exception_from_client_during_check_if_exists($dropbox) {
		$dropbox->getMetaData ( 'filename', false )->willThrow ( new \RuntimeException ( 'exists' ) );
	...
	function it_gets_keys($dropbox) {
		$dropbox->getMetaData ( '/', true )->willReturn ( array (
				'contents' => array (
						array (
								'path' => '/filename' 
						),
	...
	function it_does_not_mask_exception_from_client_during_getting_keys($dropbox) {
		$dropbox->getMetaData ( '/', true )->willThrow ( new \RuntimeException ( 'keys' ) );
	...
	function it_writes_file($dropbox) {
		$dropbox->putFile ( 'filename', Argument::any () )->shouldBeCalled ();
	...
	function it_deletes_file($dropbox) {
		$dropbox->delete ( 'filename' )->shouldBeCalled ();
	...
	function it_renames_file($dropbox) {
		$dropbox->move ( 'filename', 'filename2' )->shouldBeCalled ();
