...
class AmazonS3Spec extends ObjectBehavior {
	...
	function it_reads_file($service) {
		...
		$service->get_object ( 'bucketName', 'filename', $options )->shouldBeCalled ()->willReturn ( new \CFResponse ( 'header', 'some content', 200 ) );
	...
	function it_returns_false_when_cannot_read($service) {
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->get_object ( 'bucketName', 'filename', array () )->shouldBeCalled ()->willReturn ( new \CFResponse ( 'header', 'some content', 500 ) );
		$service->set_region ( Argument::any () )->shouldBeCalled ();
	...
	function it_rename_file($service) {
		...
		) )->shouldBeCalled ()->willReturn ( new \CFResponse ( 'header', 'some content', 200 ) );
		$service->delete_object ( 'bucketName', 'filename1', Argument::any () )->shouldBeCalled ()->willReturn ( new \CFResponse ( array (), 'some', 200 ) );
		
		$this->setMetadata ( 'filename1', array (
				'acl' => \AmazonS3::ACL_OWNER_READ 
		) );
	...
	function it_is_verbose_and_throws_exceptions_when_rename($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->copy_object ( Argument::cetera () )->willThrow ( new \RuntimeException ( 'rename' ) );
	...
	function it_returns_false_when_cannot_rename($service) {
		...
		$service->copy_object ( array (
				...
				'bucket' => 'bucketName',
				'filename' => 'filename2' 
		), array () )->shouldBeCalled ()->willReturn ( new \CFResponse ( 'header', 'some content', 500 ) );
	...
	function it_should_write_file($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->create_object ( 'bucketName', 'filename', array (
				'acl' => \AmazonS3::ACL_PRIVATE,
				'body' => 'some content' 
		) )->shouldBeCalled ()->willReturn ( new \CFResponse ( array (
				'x-aws-requestheaders' => array (
						'Content-Length' => 12 
				) 
		), 'some content', 200 ) );
		
		$this->setMetadata ( 'filename', array (
				'acl' => \AmazonS3::ACL_PRIVATE,
				'body' => 'other content' 
		) );
	...
	function it_is_verbose_and_throws_exceptions_when_write($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->create_object ( Argument::cetera () )->willThrow ( new \RuntimeException ( 'write' ) );
	...
	function it_is_verbose_and_throws_exceptions_when_file_exists($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->if_object_exists ( 'bucketName', 'filename' )->willThrow ( new \RuntimeException ( 'exists' ) );
	...
	function it_should_get_file_mtime($service) {
		...
		$service->get_object_metadata ( 'bucketName', 'filename', $metadata )->shouldBeCalled ()->willReturn ( array (
				'Headers' => array (
						'last-modified' => '2012-01-01 23:10:10' 
				) 
		) );
	...
	function it_returns_false_when_cannot_fetch_mtime($service) {
		...
		$service->get_object_metadata ( 'bucketName', 'filename', array () )->shouldBeCalled ()->willReturn ( array (
				'Headers' => array () 
		) );
	...
	function it_should_delete_file($service) {
		...
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->delete_object ( 'bucketName', 'filename', $metadata )->willReturn ( new \CFResponse ( array (), 'some', 200 ) );
	...
	function it_is_verbose_and_throws_exceptions_when_fetch_delete($service) {
		...
		$service->delete_object ( 'bucketName', 'filename', Argument::any () )->willThrow ( new \RuntimeException ( 'delete' ) );
	...
	function it_returns_false_when_cannot_delete($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->delete_object ( 'bucketName', 'filename', array () )->willReturn ( new \CFResponse ( array (), 'some', 500 ) );
	...
	function it_should_get_keys($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->shouldBeCalled ()->willReturn ( true );
		$service->get_object_list ( 'bucketName' )->shouldBeCalled ()->willReturn ( array (
				'filename2',
				'aaa/filename',
				'filename1' 
		) );
	...
	function it_should_handle_dirs($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->willReturn ( true );
		$service->if_object_exists ( 'bucketName', 'filename' )->shouldNotBeCalled ();
		$service->if_object_exists ( 'bucketName', 'filename/' )->shouldBeCalled ()->willReturn ( false );
		$service->if_object_exists ( 'bucketName', 'dirname/' )->willReturn ( true );
	...
	function it_should_fail_when_bucket_does_not_exist($service) {
		...
		$this->shouldThrow ( new \RuntimeException ( 'The configured bucket "bucketName" does not exist.' ) )->duringRead ( 'filename' );
		$this->shouldThrow ( new \RuntimeException ( 'The configured bucket "bucketName" does not exist.' ) )->duringWrite ( 'filename', 'content' );
		$this->shouldThrow ( new \RuntimeException ( 'The configured bucket "bucketName" does not exist.' ) )->duringDelete ( 'filename' );
		$this->shouldThrow ( new \RuntimeException ( 'The configured bucket "bucketName" does not exist.' ) )->duringExists ( 'filename' );
		$this->shouldThrow ( new \RuntimeException ( 'The configured bucket "bucketName" does not exist.' ) )->duringMtime ( 'filename' );
		$this->shouldThrow ( new \RuntimeException ( 'The configured bucket "bucketName" does not exist.' ) )->duringRename ( 'filename', 'filename2' );
		$this->shouldThrow ( new \RuntimeException ( 'The configured bucket "bucketName" does not exist.' ) )->duringKeys ();
	}
	...
	function it_creates_bucket_if_create_mode_is_enabled($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->willReturn ( false );
		$service->create_bucket ( 'bucketName', \AmazonS3::REGION_US_E1 )->shouldBeCalled ()->willReturn ( new \CFResponse ( array (), 'created', 201 ) );
		$service->if_object_exists ( 'bucketName', 'filename' )->willReturn ( false );
		
		$this->beConstructedWith ( $service, 'bucketName', array (
				'create' => true 
		) );
	...
	function it_fails_when_cannot_create_bucket($service) {
		$service->set_region ( Argument::any () )->shouldBeCalled ();
		$service->if_bucket_exists ( 'bucketName' )->willReturn ( false );
		$service->create_bucket ( 'bucketName', \AmazonS3::REGION_US_E1 )->shouldBeCalled ()->willReturn ( new \CFResponse ( array (), 'created', 500 ) );
		
		$this->beConstructedWith ( $service, 'bucketName', array (
				'create' => true 
		) );
		$this->shouldThrow ( new \RuntimeException ( 'Failed to create the configured bucket "bucketName".' ) )->duringExists ( 'filename' );
	}
