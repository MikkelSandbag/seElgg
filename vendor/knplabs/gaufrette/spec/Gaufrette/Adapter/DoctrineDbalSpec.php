...
class DoctrineDbalSpec extends ObjectBehavior {
	...
	function it_checks_if_file_exists($connection) {
		...
		$connection->fetchColumn ( 'SELECT COUNT("key") FROM "someTableName" WHERE "key" = :key', array (
				'key' => 'filename' 
		) )->willReturn ( 12 );
	...
	function it_writes_to_new_file($connection) {
		...
		$connection->insert ( 'someTableName', array (
	...
	function it_write_file($connection) {
		...
		$connection->update ( 'someTableName', array (
				'"content"' => 'some content',
				'"mtime"' => strtotime ( '2012-10-10 23:10:10' ),
				'"checksum"' => '9893532233caff98cd083a116b013c0b' 
		), array (
	...
	function it_reads_file($connection) {
		...
		$connection->fetchColumn ( 'SELECT "content" FROM "someTableName" WHERE "key" = :key', array (
				'key' => 'filename' 
	...
	function it_calculates_checksum($connection) {
		...
		$connection->fetchColumn ( 'SELECT "checksum" FROM "someTableName" WHERE "key" = :key', array (
				'key' => 'filename' 
		) )->willReturn ( 1234 );
	...
	function it_gets_mtime($connection) {
		...
		$connection->fetchColumn ( 'SELECT "mtime" FROM "someTableName" WHERE "key" = :key', array (
				'key' => 'filename' 
		) )->willReturn ( 1234 );
	...
	function it_get_keys($connection, $stmt) {
		$stmt->fetchAll ( \PDO::FETCH_COLUMN )->willReturn ( array (
				'filename',
				'filename1',
				'filename2' 
		) );
	...
	function it_deletes_file($connection) {
		...
		$connection->delete ( 'someTableName', array (
				'"key"' => 'filename' 
		) )->shouldBeCalled ()->willReturn ( 1 );
