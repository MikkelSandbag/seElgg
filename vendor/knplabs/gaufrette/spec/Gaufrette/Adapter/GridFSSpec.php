...
class GridFSSpec extends ObjectBehavior {
	...
	function it_does_not_fail_when_cannot_read($gridFs) {
		$gridFs->findOne ( 'filename', array () )->shouldBeCalled ()->willReturn ( null );
	...
	function it_checks_if_file_exists($gridFs, $file) {
		$gridFs->findOne ( 'filename', array () )->willReturn ( $file );
		$gridFs->findOne ( 'filename2', array () )->willReturn ( null );
	...
	function it_deletes_file($gridFs) {
		...
		$gridFs->findOne ( 'filename', array (
				'_id' 
		) )->willReturn ( $file );
		$gridFs->delete ( 123 )->willReturn ( true );
	...
	function it_does_not_delete_file($gridFs) {
		...
		$gridFs->delete ( 123 )->willReturn ( false );
	...
	function it_writes_file($gridFs, $file) {
		...
		$gridFs->findOne ( 'filename', array () )->willReturn ( null );
		$gridFs->storeBytes ( 'some content', array (
				'date' => 1234,
				'someother' => 'metadata',
				'filename' => 'filename' 
		) )->willReturn ( 'someId' );
	...
	function it_updates_file($gridFs, $file) {
		...
		$gridFs->findOne ( 'filename', array (
				'_id' 
		) )->shouldBeCalled ()->willReturn ( $someFile );
		$gridFs->delete ( 123 )->shouldBeCalled ()->willReturn ( true );
		
		$file->getSize ()->willReturn ( 12 );
		$gridFs->findOne ( 'filename', array () )->willReturn ( $file );
		$gridFs->storeBytes ( Argument::any (), Argument::any () )->willReturn ( 'someId' );
		$gridFs->findOne ( array (
				'_id' => 'someId' 
		) )->willReturn ( $file );
	...
	function it_renames_file($gridFs, $file) {
		...
		) )->shouldBeCalled ()->willReturn ( 'someId' );
		
		$fileToDelete = new \stdClass ();
		$fileToDelete->file = array (
				'_id' => 123 
		);
		$gridFs->findOne ( 'filename', array (
				'_id' 
		...
		$this->setMetadata ( 'otherFilename', array (
				'date' => 1234 
		) );
	...
	function it_fetches_keys($gridFs, $file, $otherFile) {
		$gridFs->find ( array (), array (
				'filename' 
		) )->willReturn ( array (
				new TestFile ( 'filename' ),
				new TestFile ( 'otherFilename' ) 
		) );
