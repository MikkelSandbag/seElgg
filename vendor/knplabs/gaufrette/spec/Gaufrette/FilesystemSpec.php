...
class FilesystemSpec extends ObjectBehavior {
	...
	function it_fails_when_renamed_source_file_does_not_exist($adapter) {
		...
		$this->shouldThrow ( new \Gaufrette\Exception\FileNotFound ( 'filename' ) )->duringRename ( 'filename', 'otherFilename' );
	}
	...
	function it_fails_when_renamed_target_file_exists($adapter) {
		...
		$this->shouldThrow ( new \Gaufrette\Exception\UnexpectedFile ( 'otherFilename' ) )->duringRename ( 'filename', 'otherFilename' );
	}
	...
	function it_does_not_get_file_object_when_file_with_key_does_not_exist($adapter) {
		...
		$this->shouldThrow ( new \Gaufrette\Exception\FileNotFound ( 'filename' ) )->duringGet ( 'filename' );
	}
	...
	function it_does_not_update_content_of_file_when_file_cannot_be_overwriten($adapter) {
		...
		$this->shouldThrow ( new \Gaufrette\Exception\FileAlreadyExists ( 'filename' ) )->duringWrite ( 'filename', 'some content to write' );
	}
	...
	function it_does_not_read_file_which_does_not_exist($adapter) {
		...
		$this->shouldThrow ( new \Gaufrette\Exception\FileNotFound ( 'filename' ) )->duringRead ( 'filename' );
	}
	...
	function it_fails_when_read_is_not_successful($adapter) {
		...
		$this->shouldThrow ( new \RuntimeException ( 'Could not read the "filename" key content.' ) )->duringRead ( 'filename' );
	}
	...
	function it_does_not_delete_file_which_does_not_exist($adapter) {
		...
		$this->shouldThrow ( new \Gaufrette\Exception\FileNotFound ( 'filename' ) )->duringDelete ( 'filename' );
	}
	...
	function it_match_listed_keys_using_specified_pattern($adapter) {
		...
		$this->listKeys ( 'filename' )->shouldReturn ( array (
				'keys' => array (
						'filename',
						'filename1',
						'filename2' 
		...
		$this->listKeys ( 'Key' )->shouldReturn ( array (
				'keys' => array (
						'KeyTest' 
				),
				'dirs' => array () 
	...
	function it_does_not_get_mtime_of_file_which_does_not_exist($adapter) {
		...
		$this->shouldThrow ( new \Gaufrette\Exception\FileNotFound ( 'filename' ) )->duringMtime ( 'filename' );
	}
	...
	function it_cannot_resolve_mime_type_if_the_adapter_cannot_provide_it($adapter) {
		$adapter->exists ( 'filename' )->willReturn ( true );
		$this->shouldThrow ( new \LogicException ( sprintf ( 'Adapter "%s" cannot provide MIME type', get_class ( $adapter->getWrappedObject () ) ) ) )->duringMimeType ( 'filename' );
	}
}
interface Adapter extends \Gaufrette\Adapter, \Gaufrette\Adapter\FileFactory, \Gaufrette\Adapter\StreamFactory, \Gaufrette\Adapter\ChecksumCalculator, \Gaufrette\Adapter\MetadataSupporter, \Gaufrette\Adapter\MimeTypeProvider {
}
