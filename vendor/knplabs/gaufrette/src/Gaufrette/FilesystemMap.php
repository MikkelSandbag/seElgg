...
class FilesystemMap {
	...
	public function set($domain, Filesystem $filesystem) {
		if (! preg_match ( '/^[-_a-zA-Z0-9]+$/', $domain )) {
			throw new \InvalidArgumentException ( sprintf ( 'The specified domain "%s" is not a valid domain.', $domain ) );
		}
	 ...
	 * @param string $domain        	
	 *
	 * @return Filesystem @throw InvalidArgumentException when there is no filesystem registered
	 *         for the specified domain
	 */
	public function get($domain) {
		if (! $this->has ( $domain )) {
			throw new \InvalidArgumentException ( sprintf ( 'There is no filesystem defined for the "%s" domain.', $domain ) );
		}
	...
	public function remove($domain) {
		if (! $this->has ( $domain )) {
			throw new \InvalidArgumentException ( sprintf ( 'Cannot remove the "%s" filesystem as it is not defined.', $domain ) );
		}
