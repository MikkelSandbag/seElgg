...
class Local implements Adapter, StreamFactory, ChecksumCalculator, SizeCalculator, MimeTypeProvider {
	...
	public function keys() {
		...
		try {
			$files = new \RecursiveIteratorIterator ( new \RecursiveDirectoryIterator ( $this->directory, \FilesystemIterator::SKIP_DOTS | \FilesystemIterator::UNIX_PATHS ) );
		} catch ( \Exception $e ) {
	 ...
	 *
	 * @param string $key
	 *        	The key which for to compute the path
	 *        	
	 ...
	 * @param boolean $create
	 *        	Whether to create the directory if it does
	 *        	not exist
	 *        	
