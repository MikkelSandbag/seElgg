...
class MogileFS implements Adapter {
	 ...
	 * @param
	 *        	hosts Array of MogileFS trackers
	 */
	...
	public function write($key, $content, array $metadata = null) {
		...
		if (mb_strlen ( $content ) > 0) {
			$res = $this->doRequest ( "CREATE_OPEN", array (
					"key" => $key,
	...
	private function doRequest($cmd, $args = array()) {
		...
		if ($words [0] == 'OK') {
			...
			throw new \RuntimeException ( sprintf ( 'Error response: %s', $line ), $errorCode );
		}
	...
	private function getPaths($key) {
		$res = $this->doRequest ( "GET_PATHS", array (
				"key" => $key 
		) );
		unset ( $res ['paths'] );
