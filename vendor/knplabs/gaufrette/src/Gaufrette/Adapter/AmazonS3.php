...
class AmazonS3 implements Adapter, MetadataSupporter {
	...
	public function __construct(AmazonClient $service, $bucket, $options = array()) {
		...
		$this->options = array_replace_recursive ( array (
				'directory' => '',
				'create' => false,
				'region' => AmazonClient::REGION_US_E1,
				'acl' => AmazonClient::ACL_PUBLIC 
		), $options );
	}
	...
	public function rename($sourceKey, $targetKey) {
		...
		$response = $this->service->copy_object ( array ( // source
				'bucket' => $this->bucket,
	...
	public function write($key, $content) {
		...
		$opt = array_replace_recursive ( array (
				'acl' => $this->options ['acl'] 
		), $this->getMetadata ( $key ), array (
				'body' => $content 
		) );
		
		$response = $this->service->create_object ( $this->bucket, $this->computePath ( $key ), $opt );
	...
	public function mtime($key) {
		...
		$response = $this->service->get_object_metadata ( $this->bucket, $this->computePath ( $key ), $this->getMetadata ( $key ) );
	...
	public function delete($key) {
		...
		$response = $this->service->delete_object ( $this->bucket, $this->computePath ( $key ), $this->getMetadata ( $key ) );
	...
	private function ensureBucketExists() {
		...
		$response = $this->service->create_bucket ( $this->bucket, $this->options ['region'] );
		
		if (! $response->isOK ()) {
			throw new \RuntimeException ( sprintf ( 'Failed to create the configured bucket "%s".', $this->bucket ) );
		}
