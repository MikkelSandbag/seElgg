 ...
 */
class GridFS implements Adapter, ChecksumCalculator, MetadataSupporter, ListKeysAware {
	...
	public function write($key, $content) {
		...
		$metadata = array_replace_recursive ( array (
				'date' => new MongoDate () 
		), $this->getMetadata ( $key ), array (
				'filename' => $key 
		) );
		$id = $this->gridFS->storeBytes ( $content, $metadata );
		$file = $this->gridFS->findOne ( array (
				'_id' => $id 
		) );
	...
	public function keys() {
		$keys = array ();
		$cursor = $this->gridFS->find ( array (), array (
				'filename' 
		) );
