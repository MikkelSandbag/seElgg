...
class DoctrineDbal implements Adapter, ChecksumCalculator, ListKeysAware {
	...
	public function keys() {
		$keys = array ();
		$stmt = $this->connection->executeQuery ( sprintf ( 'SELECT %s FROM %s', $this->getQuotedColumn ( 'key' ), $this->getQuotedTable () ) );
	...
	public function rename($sourceKey, $targetKey) {
		return ( boolean ) $this->connection->update ( $this->table, array (
	...
	public function exists($key) {
		return ( boolean ) $this->connection->fetchColumn ( sprintf ( 'SELECT COUNT(%s) FROM %s WHERE %s = :key', $this->getQuotedColumn ( 'key' ), $this->getQuotedTable (), $this->getQuotedColumn ( 'key' ) ), array (
				'key' => $key 
		) );
	}
	...
	public function delete($key) {
		return ( boolean ) $this->connection->delete ( $this->table, array (
	...
	public function write($key, $content) {
		...
		if ($this->exists ( $key )) {
			$this->connection->update ( $this->table, $values, array (
					$this->getQuotedColumn ( 'key' ) => $key 
			) );
		} else {
	...
	private function getColumnValue($key, $column) {
		$value = $this->connection->fetchColumn ( sprintf ( 'SELECT %s FROM %s WHERE %s = :key', $this->getQuotedColumn ( $column ), $this->getQuotedTable (), $this->getQuotedColumn ( 'key' ) ), array (
				'key' => $key 
		) );
	...
	public function listKeys($prefix = '') {
		...
		$keys = $this->connection->fetchAll ( sprintf ( 'SELECT %s AS _key FROM %s WHERE %s LIKE :pattern', $this->getQuotedColumn ( 'key' ), $this->getQuotedTable (), $this->getQuotedColumn ( 'key' ) ), array (
