...
/**
 * Stash Drivers are the engines behind the Stash library.
 * These classes handle the low level operations- retrieving,
...
interface DriverInterface {
	/**
	 * Takes an array which is used to pass option values to the driver.
	 * As this is the only required function that is
	...
	/**
	 * Returns the previously stored data as well as it's expiration date in an associative array.
	 * This array contains
	...
	/**
	 * Clears the cache tree using the key array provided as the key.
	 * If called with no arguments the entire cache gets
	 * cleared.
	 *
	...
	/**
	 * Removed any expired code from the cache.
	 * For same drivers this can just return true, as their underlying engines
	...
	/**
	 * Returns whether the driver is able to run in the current environment or not.
	 * Any system checks - such as making
