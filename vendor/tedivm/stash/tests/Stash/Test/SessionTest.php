...
class SessionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testReadAndWrite() {
		...
		$this->assertSame ( '', $session->read ( 'session_id' ), 'Empty session returns empty string.' );
		
		$this->assertTrue ( $session->write ( 'session_id', 'session_data' ), 'Data was written to the session.' );
		$this->assertSame ( 'session_data', $session->read ( 'session_id' ), 'Active session returns session data.' );
	}
	public function testOpen() {
		...
		$this->assertTrue ( $DataA != $DataB, 'Sessions with different paths do not share data.' );
		...
		$this->assertTrue ( $DataA != $DataB, 'Sessions with different names do not share data.' );
	}
	public function testClose() {
		$session = $this->getSession ();
		$this->assertTrue ( $session->close (), 'Session was closed' );
	}
	public function testDestroy() {
		...
		$this->assertTrue ( $session->destroy ( 'session_id' ), 'Data was removed from the session.' );
		
		$this->assertSame ( '', $session->read ( 'session_id' ), 'Destroyed session returns empty string.' );
	}
	public function testGarbageCollect() {
		$pool = $this->getPool ();
		
		$sessionA = $this->getSession ( $pool );
		$sessionA->setOptions ( array (
				'ttl' => - 30 
		) );
		...
		$this->assertSame ( '', $sessionC->read ( 'session_id' ), 'Purged session returns empty string.' );
	}
