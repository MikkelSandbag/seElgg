...
if (isset ( $_POST ['url'] )) {
	...
	if ($type ['minify'] === 'text/html' && isset ( $_POST ['addBase'] ) && ! preg_match ( '@<base\\b@i', $content )) {
		$content = preg_replace ( '@(<head\\b[^>]*>)@i', '$1<base href="' . htmlspecialchars ( $url, ENT_QUOTES, 'UTF-8' ) . '" />', $content );
	}
	...
	if ($type ['minify'] === 'text/html') {
		if (isset ( $_POST ['minJs'] )) {
			$sourceSpec ['minifyOptions'] ['jsMinifier'] = array (
					'JSMin',
					'minify' 
			);
		}
		if (isset ( $_POST ['minCss'] )) {
			$sourceSpec ['minifyOptions'] ['cssMinifier'] = array (
					'Minify_CSS',
					'minify' 
			);
		}
...
<p>
	<strong>Warning! Please do not place this application on a public site.</strong>
	This should be used only for testing.
</p>

<h1>Fetch and Minify a URL</h1>
<p>This tool will retrieve the contents of a URL and minify it. The
	fetched resource Content-Type will determine the minifier used.</p>
...
<form action="?2" method="post">
	...
	<p>
		<input type="submit" value="Fetch and minify">
	</p>

	<fieldset>
		<legend>HTML options</legend>
		<p>If the resource above is sent with an (x)HTML Content-Type, the
			following options will apply:</p>
		<ul>
			<li><label><input type="checkbox" name="asText" checked> Return plain
					text (o/w send the original content type)</label>
			...
			<li><label><input type="checkbox" name="addBase" checked> Add BASE
					element (if not present)</label>
	...
	<fieldset>
		...
		<ul>
			<li><label>User-Agent: <input type="text" name="ua" size="60"
