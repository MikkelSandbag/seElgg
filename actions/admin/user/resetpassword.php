...
if (($user instanceof ElggUser) && ($user->canEdit ())) {
	...
	if (force_user_password_reset ( $user->guid, $password )) {
		...
		notify_user ( $user->guid, elgg_get_site_entity ()->guid, elgg_echo ( 'email:resetpassword:subject', array (), $user->language ), elgg_echo ( 'email:resetpassword:body', array (
				$user->username,
				$password 
		), $user->language ), array (), 'email' );
	} else {
