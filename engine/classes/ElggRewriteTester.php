...
class ElggRewriteTester {
	 ...
	 *        	URL of rewrite test
	 * @param string $path
	 *        	Root directory of Elgg with trailing slash
	 *        	
	 * @return array
	 */
	...
	/**
	 * Create Elgg's .
	 * htaccess file or confirm that it exists
	 *
	 * @param string $url
	 *        	URL of rewrite test
	 * @param string $path
	 *        	Elgg's root directory with trailing slash
	 *        	
	 * @return bool
	 */
	...
	protected function returnStatus($url) {
		...
		if ($this->serverSupportsRemoteRead == FALSE) {
			$msg = _elgg_services ()->translator->translate ( 'install:warning:rewrite:unknown', array (
					$url 
			) );
			$msg .= elgg_view ( 'install/js_rewrite_check', array (
					'url' => $url 
			) );
