...
class ElggAutoP {
	...
	public function process($html) {
		...
		if (! $this->_doc->loadHTML ( "<html><meta http-equiv='content-type' " . "content='text/html; charset={$this->encoding}'><body>{$html}</body>" . "</html>" )) {
		...
		while ( $el = array_shift ( $elsToProcess ) ) {
			...
			while ( null !== $node ) {
				...
				if ($alterInline) {
					...
					$isLastInline = (! $node->nextSibling || ($node->nextSibling->nodeType === XML_ELEMENT_NODE && in_array ( $node->nextSibling->nodeName, $this->_blocks )));
					if ($isElement) {
