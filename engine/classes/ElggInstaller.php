...
class ElggInstaller {
	...
	public function run($step) {
		...
		if (! in_array ( $step, $this->getSteps () )) {
			$msg = _elgg_services ()->translator->translate ( 'InstallationException:UnknownStep', array (
					$step 
			) );
	 ...
	 * @param bool $createHtaccess
	 *        	Should .htaccess be created
	 *        	
	 * @return void
	 * @throws InstallationException
	 */
	public function batchInstall(array $params, $createHtaccess = FALSE) {
		...
		foreach ( $requiredParams as $key ) {
			if (empty ( $params [$key] )) {
				$msg = _elgg_services ()->translator->translate ( 'install:error:requiredfield', array (
						$key 
				) );
				throw new InstallationException ( $msg );
			}
	 ...
	 *        	The current step
	 * @param array $vars
	 *        	Array of vars to pass to the view
	 *        	
	 * @return void
	 */
	...
	protected function admin($submissionVars) {
		...
		$this->CONFIG->translations [$lang] ['install:admin:help:password1'] = sprintf ( $this->CONFIG->translations [$lang] ['install:admin:help:password1'], $this->CONFIG->min_password_length );
	...
	protected function setInstallStatus() {
		...
		if (! include_once ("{$this->CONFIG->path}engine/lib/database.php")) {
			throw new InstallationException ( _elgg_services ()->translator->translate ( 'InstallationException:MissingLibrary', array (
					'database.php' 
			) ) );
		}
	 ...
	 *
	 * @param string $step
	 *        	Installation step to check against
	 *        	
	 * @return void
	 */
	 ...
	 * out where to continue from.
	 * Returns the best guess on the step.
	 *
	 * @param string $step
	 *        	Installation step to resume from
	 *        	
	 * @return string
	 */
	 ...
	 *
	 * @param string $step
	 *        	Which step to boot strap for. Required because
		...
		if ($stepIndex > $dbIndex) {
			...
			foreach ( $lib_files as $file ) {
				...
				if (! include_once ($path)) {
					throw new InstallationException ( 'InstallationException:MissingLibrary', array (
							$file 
					) );
				}
			}
	...
	protected function bootstrapConfig() {
		...
		$this->CONFIG->entity_types = array (
				'group',
				'object',
				'site',
				'user' 
	...
	protected function checkPHP(&$report) {
		...
		if (version_compare ( PHP_VERSION, $min_php_version, '<' )) {
			$phpReport [] = array (
					'severity' => 'failure',
					'message' => _elgg_services ()->translator->translate ( 'install:check:php:version', array (
							$min_php_version,
							PHP_VERSION 
					) ) 
			);
	...
	protected function checkPhpExtensions(&$phpReport) {
		...
		foreach ( $requiredExtensions as $extension ) {
			if (! in_array ( $extension, $extensions )) {
				$phpReport [] = array (
						'severity' => 'failure',
						'message' => _elgg_services ()->translator->translate ( 'install:check:php:extension', array (
								$extension 
						) ) 
				);
			}
		...
		foreach ( $recommendedExtensions as $extension ) {
			if (! in_array ( $extension, $extensions )) {
				$phpReport [] = array (
						'severity' => 'warning',
						'message' => _elgg_services ()->translator->translate ( 'install:check:php:extension:recommend', array (
								$extension 
						) ) 
				);
			}
	...
	protected function checkPhpDirectives(&$phpReport) {
		...
		if (ini_get ( 'arg_separator.output' ) !== '&') {
			...
			$msg = _elgg_services ()->translator->translate ( "install:check:php:arg_separator", array (
					$separator 
			) );
			$phpReport [] = array (
					'severity' => 'failure',
	...
	protected function checkRewriteRules(&$report) {
		...
		$report ['rewrite'] = array (
				$tester->run ( $url, $this->CONFIG->path ) 
		);
	}
	...
	protected function validateDatabaseVars($submissionVars, $formVars) {
		foreach ( $formVars as $field => $info ) {
			if ($info ['required'] == TRUE && ! $submissionVars [$field]) {
				...
				register_error ( _elgg_services ()->translator->translate ( 'install:error:requiredfield', array (
						$name 
				) ) );
				return FALSE;
			}
	...
	protected function checkDatabaseSettings($user, $password, $dbname, $host) {
		...
		if ($points [0] < $required_version) {
			register_error ( _elgg_services ()->translator->translate ( 'install:error:oldmysql', array (
					$version 
			) ) );
			return FALSE;
		}
		...
		if (! $result) {
			register_error ( _elgg_services ()->translator->translate ( 'install:error:nodatabase', array (
					$dbname 
			) ) );
		}
	 ...
	 *
	 * @param array $params
	 *        	Array of inputted params from the user
	 *        	
	 * @return bool
	 */
	...
	protected function validateSettingsVars($submissionVars, $formVars) {
		...
		if (stripos ( PHP_OS, 'win' ) === 0) {
			if (strpos ( $submissionVars ['dataroot'], ':' ) !== 1) {
				$msg = _elgg_services ()->translator->translate ( 'install:error:relative_path', array (
						$submissionVars ['dataroot'] 
				) );
				register_error ( $msg );
				return FALSE;
			}
		...
		if (! file_exists ( $submissionVars ['dataroot'] )) {
			$msg = _elgg_services ()->translator->translate ( 'install:error:datadirectoryexists', array (
					$submissionVars ['dataroot'] 
			) );
			register_error ( $msg );
			return FALSE;
		...
		if (! is_writable ( $submissionVars ['dataroot'] )) {
			$msg = _elgg_services ()->translator->translate ( 'install:error:writedatadirectory', array (
					$submissionVars ['dataroot'] 
			) );
			register_error ( $msg );
			return FALSE;
		...
		if (! isset ( $this->CONFIG->data_dir_override ) || ! $this->CONFIG->data_dir_override) {
			...
			if (stripos ( $submissionVars ['dataroot'], $submissionVars ['path'] ) === 0) {
				$msg = _elgg_services ()->translator->translate ( 'install:error:locationdatadirectory', array (
						$submissionVars ['dataroot'] 
				) );
				register_error ( $msg );
				return FALSE;
		...
		if ($submissionVars ['siteemail'] && ! is_email_address ( $submissionVars ['siteemail'] )) {
			$msg = _elgg_services ()->translator->translate ( 'install:error:emailaddress', array (
					$submissionVars ['siteemail'] 
			) );
			register_error ( $msg );
			return FALSE;
		}
