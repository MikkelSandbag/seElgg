...
class ElggPlugin extends \ElggObject {
	 ...
	 *        	The priority to set. One of +1, -1, first, last, or a number.
	 ...
	 *
	 * @param int $user_guid
	 *        	The user GUID. Defaults to logged in.
	 * @return array An array of key/value pairs.
	 */
	 ...
	 *      
	 * @param int $user_guid
	 *        	The user GUID to remove user settings.
	 * @return bool
	 */
	...
	public function isValid() {
		if (! $this->getID ()) {
			$this->errorMsg = _elgg_services ()->translator->translate ( 'ElggPlugin:MissingID', array (
					$this->guid 
			) );
			return false;
		}
		...
		if (! $this->getPackage () instanceof \ElggPluginPackage) {
			$this->errorMsg = _elgg_services ()->translator->translate ( 'ElggPlugin:NoPluginPackagePackage', array (
					$this->getID (),
					$this->guid 
			) );
			return false;
	 ...
	 *
	 * @param int $flags
	 *        	Start flags for the plugin. See the constants in lib/plugins.php for details.
	 * @return true
	 * @throws PluginException
	 */
	...
	protected function includeFile($filename) {
		...
		if (! $this->canReadFile ( $filename )) {
			$msg = _elgg_services ()->translator->translate ( 'ElggPlugin:Exception:CannotIncludeFile', array (
					$filename,
					$this->getID (),
					$this->guid,
					$this->path 
	...
	protected function registerViews() {
		...
		if (! $handle) {
			$msg = _elgg_services ()->translator->translate ( 'ElggPlugin:Exception:CannotRegisterViews', array (
					$this->getID (),
					$this->guid,
					$view_dir 
			) );
	...
	protected function registerLanguages() {
		...
		if (! _elgg_services ()->translator->registerTranslations ( $languages_path )) {
			$msg = _elgg_services ()->translator->translate ( 'ElggPlugin:Exception:CannotRegisterLanguages', array (
					$this->getID (),
					$this->guid,
					$languages_path 
			) );
	 ...
	 *
	 * @param string $name
	 *        	Name of the attribute or private setting
	 * @return mixed
	 */
	 ...
	 *
	 * @param string $name
	 *        	Name of the attribute or private_setting
	 * @param mixed $value
	 *        	Value to be set
	 * @return void
	 */
	 ...
	 * @param mixed $site_guid
	 *        	Int for specific site, null for current site.
	 *        	
	 * @return bool
	 */
