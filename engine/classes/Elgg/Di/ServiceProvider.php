...
class ServiceProvider extends \Elgg\Di\DiContainer {
	 ...
	 * Constructor
	 *
	 * @param \Elgg\AutoloadManager $autoload_manager
	 *        	Class autoloader
	 */
	public function __construct(\Elgg\AutoloadManager $autoload_manager) {
		...
		$this->setFactory ( 'metadataTable', function (ServiceProvider $c) {
			...
			return new \Elgg\Database\MetadataTable ( $c->metadataCache, $c->db, $c->entityTable, $c->events, $c->metastringsTable, $c->session );
		} );
		...
		$this->setFactory ( 'ownerPreloader', function (ServiceProvider $c) {
			return new \Elgg\EntityPreloader ( array (
					'owner_guid' 
			) );
		} );
		...
		$this->setFactory ( 'persistentLogin', function (ServiceProvider $c) {
			...
			return new \Elgg\PersistentLoginService ( $c->db, $c->session, $c->crypto, $cookie_config, $cookie_token );
		} );
	 ...
	 *
	 * @param string $service_needed
	 *        	The service requested first
	 * @return mixed
	 */
