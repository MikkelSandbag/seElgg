...
class NotificationsService {
	 ...
	 * Constructor
	 *
	 * @param \Elgg\Notifications\SubscriptionsService $subscriptions
	 *        	Subscription service
	 * @param \Elgg\Queue\Queue $queue
	 *        	Queue
	 * @param \Elgg\PluginHooksService $hooks
	 *        	Plugin hook service
	 * @param \Elgg\Access $access
	 *        	Access control service
	 */
	public function __construct(\Elgg\Notifications\SubscriptionsService $subscriptions, \Elgg\Queue\Queue $queue, \Elgg\PluginHooksService $hooks, \Elgg\Access $access) {
	 ...
	 *
	 * @param int $stopTime
	 *        	The Unix time to stop sending notifications
	 ...
	 *
	 * @param \Elgg\Notifications\Event $event
	 *        	Notification event
	...
	protected function sendNotification(\Elgg\Notifications\Event $event, $guid, $method) {
		...
		$subject = _elgg_services ()->translator->translate ( 'notification:subject', array (
				$actor->name 
		), $language );
		$body = _elgg_services ()->translator->translate ( 'notification:body', array (
				$object->getURL () 
		), $language );
		...
		if ($this->hooks->hasHandler ( 'send', "notification:$method" )) {
			...
			return ( bool ) _elgg_notify_user ( $userGuid, $senderGuid, $subject, $body, $params, array (
					$method 
			) );
		}
	}
