...
class ActionsService {
	...
	public function execute($action, $forwarder = "") {
		...
		if (! isset ( $this->actions [$action] )) {
			register_error ( _elgg_services ()->translator->translate ( 'actionundefined', array (
					$action 
			) ) );
			...
			if (_elgg_services ()->hooks->trigger ( 'action', $action, null, true )) {
				if (! include ($this->actions [$action] ['file'])) {
					register_error ( _elgg_services ()->translator->translate ( 'actionnotfound', array (
							$action 
					) ) );
				}
			}
		...
		if (($token) && ($ts) && ($session_id)) {
			...
			if ($token_matches) {
				if ($this->validateTokenTimestamp ( $ts )) {
					...
					if (elgg_is_xhr ()) {
						register_error ( _elgg_services ()->translator->translate ( 'js:security:token_refresh_failed', array (
