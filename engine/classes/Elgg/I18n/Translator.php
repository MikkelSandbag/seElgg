...
class Translator {
	 ...
	 *        	The short message code
	 * @param array $args
	 *        	An array of arguments to pass through vsprintf().
	 * @param string $language
	 *        	Optionally, the standard language code
	 ...
	 *        	Standard country code (eg 'en', 'nl', 'es')
	 * @param array $language_array
	 ...
	 *        	Full path
	 * @param bool $load_all
	 *        	If true all languages are loaded, if
	...
	function reloadAllTranslations() {
		...
		if ($this->CONFIG->i18n_loaded_from_cache) {
			...
			$filenames = elgg_get_file_list ( $cache_dir, array (), array (), array (
					".lang" 
			) );
	...
	function getMissingLanguageKeys($language) {
		...
		foreach ( $this->CONFIG->translations ['en'] as $k => $v ) {
			if ((! isset ( $this->CONFIG->translations [$language] [$k] )) || ($this->CONFIG->translations [$language] [$k] == $this->CONFIG->translations ['en'] [$k])) {
				$missing [] = $k;
			}
		}
