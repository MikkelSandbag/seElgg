...
class ViewsService {
			...
			while ( $view = readdir ( $handle ) ) {
				...
				if (substr ( $view, 0, 1 ) !== '.') {
					if (is_dir ( $folder . "/" . $view )) {
						$this->autoregisterViews ( $view_base_new . $view, $folder . "/" . $view, $base_location_path, $viewtype );
					} else {
						$this->setViewLocation ( $view_base_new . basename ( $view, '.php' ), $base_location_path, $viewtype );
					}
				}
			}
			return true;
	...
	public function setViewLocation($view, $location, $viewtype = '') {
		...
		if (! isset ( $this->CONFIG->views->locations )) {
			$this->CONFIG->views->locations = array (
					$viewtype => array (
							$view => $location 
					) 
			);
		} else if (! isset ( $this->CONFIG->views->locations [$viewtype] )) {
			$this->CONFIG->views->locations [$viewtype] = array (
					$view => $location 
			);
		} else {
	 ...
	 *        	The viewtype
	 * @param bool $issue_missing_notice
	 *        	Log a notice if the view is missing
	 *        	
