...
class SubtypeTable {
	...
	function add($type, $subtype, $class = "") {
		...
		if (! $id) {
			...
			$id = _elgg_services ()->db->insertData ( "INSERT INTO {$this->CONFIG->dbprefix}entity_subtypes" . " (type, subtype, class) VALUES ('$type', '$subtype', '$class')" );
	...
	function remove($type, $subtype) {
		...
		$success = _elgg_services ()->db->deleteData ( "DELETE FROM {$this->CONFIG->dbprefix}entity_subtypes" . " WHERE type = '$type' AND subtype = '$subtype'" );
