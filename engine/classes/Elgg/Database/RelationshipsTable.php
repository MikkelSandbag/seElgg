...
class RelationshipsTable {
	...
	function getEntities($options) {
		...
		if ($clauses) {
			...
			$time_wheres = _elgg_get_entity_time_where_sql ( 'r', $options ['relationship_created_time_upper'], $options ['relationship_created_time_lower'] );
			if ($time_wheres) {
				$options ['wheres'] = array_merge ( $options ['wheres'], array (
						$time_wheres 
				) );
			}
	 ...
	 * @access private
	 */
	function getEntityRelationshipWhereSql($column, $relationship = null, $relationship_guid = null, $inverse_relationship = false) {
