...
class AccessCollections {
	 ...
	 *
	 * @param int $site_guid
	 *        	The GUID of the default Elgg site
	 */
	 ...
	 *
	 * @param int $user_guid
	 *        	User ID; defaults to currently logged in user
	 * @param int $site_guid
	 *        	Site ID; defaults to current site
	 * @param bool $flush
	 *        	If set to true, will refresh the access list from the
	 ...
	 *        	Site ID; defaults to current site
	 * @param bool $flush
	 *        	If set to true, will refresh the access ids from the
	 *        	database rather than using this function's cache.
	 *        	
	...
	function getAccessArray($user_guid = 0, $site_guid = 0, $flush = false) {
		...
		if ($cache [$hash]) {
			...
			if ($user_guid) {
				...
				// Get ACL memberships
				$query = "SELECT am.access_collection_id" . " FROM {$prefix}access_collection_membership am" . " LEFT JOIN {$prefix}access_collections ag ON ag.id = am.access_collection_id" . " WHERE am.user_guid = $user_guid AND (ag.site_guid = $site_guid OR ag.site_guid = 0)";
	 ...
	 * @param \ElggUser $user
	 *        	Optionally user to check access for. Defaults to
	 ...
	 *        	The current site.
	 * @param bool $flush
	 *        	If this is set to true, this will ignore a cached access array
	 *        	
	 ...
	 *        	The user GUID to check for. Defaults to logged in user.
	 * @return bool
	 */
	 ...
	 *        	The GUID of the owner (default: currently logged in user).
	 * @param int $site_guid
	 *        	The GUID of the site (default: current site).
	 *        	
	 ...
	 *
	 * @param int $user_guid
	 *        	The GUID of the user to add
	 * @param int $collection_id
	 *        	The ID of the collection to add them to
	 *        	
	 * @return bool
	 */
	 ...
	 *        	The collection's ID
	 * @param bool $idonly
	 *        	If set to true, will only return the members' GUIDs (default: false)
	 *        	
	...
	function getMembers($collection, $idonly = false) {
		...
		if (! $idonly) {
			$query = "SELECT e.* FROM {$prefix}access_collection_membership m" . " JOIN {$prefix}entities e ON e.guid = m.user_guid" . " WHERE m.access_collection_id = {$collection}";
			$collection_members = $db->getData ( $query, "entity_row_to_elggstar" );
		} else {
			$query = "SELECT e.guid FROM {$prefix}access_collection_membership m" . " JOIN {$prefix}entities e ON e.guid = m.user_guid" . " WHERE m.access_collection_id = {$collection}";
