...
class Plugins {
	 ...
	 *
	 * @param string $dir
	 *        	A dir to scan for plugins. Defaults to config's plugins_path.
	 ...
	 * entities for them if they don't exist.
	 * If there are plugins with entities
	 ...
	 *        	The id (dir name) of the plugin. NOT the guid.
		...
		if ($plugins) {
			foreach ( $plugins as $plugin ) {
				try {
					...
					$plugin->deactivate ();
					$msg = _elgg_services ()->translator->translate ( 'PluginException:CannotStart', array (
							$plugin->getID (),
							$plugin->guid,
							$e->getMessage () 
					) );
	 ...
	 *
	 * @param string $status
	 *        	The status of the plugins. active, inactive, or all.
	 * @param mixed $site_guid
	 *        	Optional site guid
	 * @return \ElggPlugin[]
	 */
	 ...
	 *
	 * @param array $order
	 *        	An array of plugin ids in the order to set them
	 * @return bool
	 * @access private
	 */
	 ...
	 *
	 * @param string $type
	 *        	The type of setting: user_setting or internal.
	 * @param string $name
	 *        	The name to namespace.
	 * @param string $id
	 *        	The plugin's ID to namespace with. Required for user_setting.
	 * @return string
	 * @access private
	 */
	 ...
	 *
	 * @param string $type
	 *        	The type of provides to return
	 * @param string $name
	 *        	A specific provided name to return. Requires $provide_type.
	 *        	
	 * @return array
	 * @access private
	 */
	 ...
	 *
	 * @param string $type
	 *        	The type of the provide
	 * @param string $name
	 *        	The name of the provide
	 * @param string $version
	 *        	A version to check against
	 * @param string $comparison
	 *        	The comparison operator to use in version_compare()
	 *        	
	 ...
	 *
	 * @param array $dep
	 *        	An \ElggPluginPackage dependency array
	 * @return array
	 * @access private
	 */
	function getDependencyStrings($dep) {
		...
		switch ($type) {
			...
			case 'php_extension' :
				...
				$strings ['name'] = $translator->translate ( 'ElggPlugin:Dependencies:PhpExtension', array (
						$info ['name'] 
				) );
				if ($info ['version']) {
					$strings ['expected_value'] = "$comparison {$info['version']}";
			...
			case 'php_ini' :
				$strings ['name'] = $translator->translate ( 'ElggPlugin:Dependencies:PhpIni', array (
						$info ['name'] 
				) );
			...
			case 'plugin' :
				$strings ['name'] = $translator->translate ( 'ElggPlugin:Dependencies:Plugin', array (
						$info ['name'] 
				) );
			...
			case 'priority' :
				...
				$strings ['expected_value'] = $translator->translate ( "ElggPlugin:Dependencies:Priority:$expected_priority", array (
						$info ['plugin'] 
				) );
				$strings ['local_value'] = $translator->translate ( "ElggPlugin:Dependencies:Priority:$real_priority", array (
						$info ['plugin'] 
				) );
				$strings ['comment'] = '';
				break;
	 ...
	 *
	 * @param int $user_guid
	 *        	The user GUID or 0 for the currently logged in user.
	 * @param string $plugin_id
	 *        	The plugin ID (Required)
	 * @param bool $return_obj
	 *        	Return settings as an object? This can be used to in reusable
	 ...
	 *        	The plugin ID (Required)
	 * @param mixed $default
	 *        	The default value to return if none is set
	 *        	
	 * @return mixed
	 ...
	 *
	 * @param string $name
	 *        	The name of the setting - note, can't be "title".
	 * @param mixed $value
	 *        	The value.
	 * @param string $plugin_id
	 *        	The plugin ID (Required)
