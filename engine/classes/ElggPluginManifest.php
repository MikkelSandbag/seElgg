...
class ElggPluginManifest {
	 ...
	 *        	A string, XmlElement, or path of a manifest file.
	 * @param string $plugin_id
	 *        	Optional ID of the owning plugin. Used to
	...
	public function __construct($manifest, $plugin_id = null) {
		...
		if (! $manifest_obj) {
			throw new \PluginException ( _elgg_services ()->translator->translate ( 'PluginException:InvalidManifest', array (
					$this->getPluginID () 
			) ) );
		}
		...
		if ($class_exists) {
			...
			throw new \PluginException ( _elgg_services ()->translator->translate ( 'PluginException:NoAvailableParser', array (
					$this->apiVersion,
					$this->getPluginID () 
			) ) );
		}
		...
		if (! $this->parser->parse ()) {
			throw new \PluginException ( _elgg_services ()->translator->translate ( 'PluginException:ParserError', array (
					$this->apiVersion,
					$this->getPluginID () 
			) ) );
		}
	 ...
	 *
	 * @param array $struct
	 *        	The struct to normalize $element to.
	 * @param array $array
	 *        	The array
	 *        	
	 * @return array
	 ...
	 *        	The category as defined in the manifest.
