...
class ElggMemcache extends \ElggSharedMemoryCache {
	 ...
	 *        	The namespace for this cache to write to -
	...
	public function __construct($namespace = 'default') {
		...
		if (is_callable ( array (
			...
			foreach ( $this->CONFIG->memcache_servers as $server ) {
				if (is_array ( $server )) {
					$this->memcache->addServer ( $server [0], isset ( $server [1] ) ? $server [1] : 11211, isset ( $server [2] ) ? $server [2] : false, isset ( $server [3] ) ? $server [3] : 1, isset ( $server [4] ) ? $server [4] : 1, isset ( $server [5] ) ? $server [5] : 15, isset ( $server [6] ) ? $server [6] : true );
				} else {
		...
		if (version_compare ( $this->version, \ElggMemcache::$MINSERVERVERSION, '<' )) {
			$msg = vsprintf ( 'Memcache needs at least version %s to run, you are running %s', array (
	 ...
	 *
	 * @param int $expires
	 *        	The lifetime as a unix timestamp or time from now. Defaults forever.
	 *        	
	 * @return void
	 */
