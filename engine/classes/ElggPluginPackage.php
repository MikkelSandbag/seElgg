...
class ElggPluginPackage {
	 ...
	 *
	 * @param string $plugin
	 *        	The ID (directory name) or full path of the plugin.
	 * @param bool $validate
	 *        	Automatically run isValid()?
	 *        	
	...
	public function __construct($plugin, $validate = true) {
		...
		if (strpos ( $plugin, $plugin_path ) === 0 || is_dir ( $plugin )) {
			...
			if (preg_match ( '/[^a-z0-9\.\-_]/i', $plugin )) {
				throw new \PluginException ( _elgg_services ()->translator->translate ( 'PluginException:InvalidID', array (
						$plugin 
				) ) );
			}
		...
		if (! is_dir ( $path )) {
			throw new \PluginException ( _elgg_services ()->translator->translate ( 'PluginException:InvalidPath', array (
					$path 
			) ) );
		}
		...
		if ($validate && ! $this->isValid ()) {
			if ($this->errorMsg) {
				throw new \PluginException ( _elgg_services ()->translator->translate ( 'PluginException:InvalidPlugin:Details', array (
						$plugin,
						$this->errorMsg 
				) ) );
			} else {
				throw new \PluginException ( _elgg_services ()->translator->translate ( 'PluginException:InvalidPlugin', array (
						$plugin 
				) ) );
			}
		}
	...
	private function validate() {
		...
		foreach ( $this->requiredFiles as $file ) {
			if (! is_readable ( $this->path . $file )) {
				...
				$this->errorMsg = _elgg_services ()->translator->translate ( 'ElggPluginPackage:InvalidPlugin:MissingFile', array (
						$file 
				) );
				return false;
			}
	...
	private function isNamedCorrectly() {
		...
		if ($manifest) {
			...
			if (! empty ( $required_id ) && ($required_id !== $this->id)) {
				$this->errorMsg = _elgg_services ()->translator->translate ( 'ElggPluginPackage:InvalidPlugin:InvalidId', array (
						$required_id 
				) );
				return false;
			}
	 ...
	 * or something provides.
	 * Also check that it only list
	...
	private function hasSaneDependencies() {
		...
		foreach ( $provides as $provide ) {
			...
			if (! in_array ( $provide ['type'], $this->providesSupportedTypes )) {
				$this->errorMsg = _elgg_services ()->translator->translate ( 'ElggPluginPackage:InvalidPlugin:InvalidProvides', array (
						$provide ['type'] 
				) );
				return false;
			}
			...
			foreach ( array (
				...
				foreach ( ${$dep_type} as $dep ) {
					if (! in_array ( $dep ['type'], $this->depsSupportedTypes )) {
						$this->errorMsg = _elgg_services ()->translator->translate ( 'ElggPluginPackage:InvalidPlugin:InvalidDependency', array (
								$dep ['type'] 
						) );
						return false;
					}
					...
					if (isset ( $dep ['name'] ) && $dep ['name'] == $name) {
						...
						if ($version_compare) {
							$this->errorMsg = _elgg_services ()->translator->translate ( 'ElggPluginPackage:InvalidPlugin:CircularDep', array (
									$dep ['type'],
									$dep ['name'],
									$this->id 
							) );
	 ...
	 *
	 * @param array $dep
	 *        	An Elgg manifest.xml deps array
	 * @param array $plugins
	 *        	A list of plugins as returned by elgg_get_plugins();
	 * @param bool $inverse
	 *        	Inverse the results to use as a conflicts.
	 * @return bool
	 */
	 ...
	 * @param array $dep
	 *        	An Elgg manifest.xml deps array
	 * @param array $elgg_version
	 *        	An Elgg version (either YYYYMMDDXX or X.Y.Z)
	 * @param bool $inverse
	 *        	Inverse the result to use as a conflicts.
	 * @return bool
	 */
