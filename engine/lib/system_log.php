 ...
 *      
 * @param int|array $by_user
 *        	The guid(s) of the user(s) who initiated the event.
 *        	Use 0 for unowned entries. Anything else falsey means anyone.
 * @param string $event
 *        	The event you are searching on.
 * @param string $class
 *        	The class of object it effects.
 * @param string $type
 *        	The type
 * @param string $subtype
 *        	The subtype.
 * @param int $limit
 *        	Maximum number of responses to return. (default from settings)
 * @param int $offset
 ...
 * @return mixed
 */
function get_system_log($by_user = "", $event = "", $class = "", $type = "", $subtype = "", $limit = null, $offset = 0, $count = false, $timebefore = 0, $timeafter = 0, $object_id = 0, $ip_address = "") {
	global $CONFIG;
 ...
 *
 * @param \stdClass|int $entry
 *        	The log entry row or its ID
 *        	
 * @return mixed
 */
 ...
 *
 * @param int $offset
 *        	An offset in seconds from now to archive (useful for log rotation)
 *        	
 * @return bool
 */
...
function system_log_listener($event, $object_type, $object) {
	if (($object_type != 'systemlog') && ($event != 'log')) {
		elgg_trigger_event ( 'log', 'systemlog', array (
				'object' => $object,
				'event' => $event 
		) );
	}
