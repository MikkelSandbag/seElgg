...
function elgg_create_river_item(array $options = array()) {
	...
	if ($id) {
		...
		$river_items = elgg_get_river ( array (
				'id' => $id 
		) );
		if ($river_items) {
...
function elgg_get_river(array $options = array()) {
	...
	$object_access_where = _elgg_get_access_where_sql ( array (
			'table_alias' => 'oe' 
	) );
	$target_access_where = _elgg_get_access_where_sql ( array (
			'table_alias' => 'te' 
	) );
 ...
 *
 * @param array $options
 *        	Any options from elgg_get_river() plus:
 ...
 *
 * @param \stdClass $row
 *        	Database row from the river table
 *        	
...
function _elgg_get_river_type_subtype_where_sql($table, $types, $subtypes, $pairs) {
	...
	if (! is_array ( $pairs )) {
		...
		if (is_array ( $types_wheres ) && count ( $types_wheres )) {
			$types_wheres = array (
					implode ( ' OR ', $types_wheres ) 
			);
		}
		
		if (is_array ( $subtypes_wheres ) && count ( $subtypes_wheres )) {
			$subtypes_wheres = array (
					'(' . implode ( ' OR ', $subtypes_wheres ) . ')' 
			);
		}
		
		$wheres = array (
				implode ( ' AND ', array_merge ( $types_wheres, $subtypes_wheres ) ) 
		);
	} else {
		...
		foreach ( $pairs as $paired_type => $paired_subtypes ) {
			...
			if (is_array ( $paired_subtypes )) {
				...
				if ($paired_subtype_str) {
					$wheres [] = "({$table}.type = '$paired_type'" . " AND {$table}.subtype IN ('$paired_subtype_str'))";
				}
			} else {
				$paired_subtype = sanitise_string ( $paired_subtypes );
				$wheres [] = "({$table}.type = '$paired_type'" . " AND {$table}.subtype = '$paired_subtype')";
			}
		}
	}
 ...
 *        	The event 'disable'
 * @param string $type
 *        	Type of entity being disabled 'all'
 * @param mixed $entity
 *        	The entity being disabled
 * @return boolean
 * @access private
 */
 ...
 *        	The event 'enable'
 * @param string $type
 *        	Type of entity being enabled 'all'
 * @param mixed $entity
 *        	The entity being enabled
 * @return boolean
 * @access private
 */
