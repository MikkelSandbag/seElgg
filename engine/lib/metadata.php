 ...
 *
 * @param int $id
 *        	The id of the metadata object being retrieved.
 *        	
 ...
 *        	The entity to attach the metadata to
 * @param string $name
 ...
 *        	GUID of entity that owns the metadata. Default is logged in user.
 * @param int $access_id
 *        	Default is ACCESS_PRIVATE
 * @param bool $allow_multiple
 *        	Allow multiple values for one key. Default is false
 *        	
 * @return int|false id of metadata or false if failure
 */
function create_metadata($entity_guid, $name, $value, $value_type = '', $owner_guid = 0, $access_id = ACCESS_PRIVATE, $allow_multiple = false) {
	return _elgg_services ()->metadataTable->create ( $entity_guid, $name, $value, $value_type, $owner_guid, $access_id, $allow_multiple );
}
...
function update_metadata($id, $name, $value, $value_type, $owner_guid, $access_id) {
	return _elgg_services ()->metadataTable->update ( $id, $name, $value, $value_type, $owner_guid, $access_id );
}
 ...
 * @param int $entity_guid
 *        	The entity to attach the metadata to
 * @param array $name_and_values
 *        	Associative array - a value can be a string, number, bool
 * @param string $value_type
 *        	'text', 'integer', or '' for automatic detection
 * @param int $owner_guid
...
function create_metadata_from_array($entity_guid, array $name_and_values, $value_type, $owner_guid, $access_id = ACCESS_PRIVATE, $allow_multiple = false) {
	return _elgg_services ()->metadataTable->createFromArray ( $entity_guid, $name_and_values, $value_type, $owner_guid, $access_id, $allow_multiple );
}

/**
 * Returns metadata.
 * Accepts all elgg_get_entities() options for entity
 * restraints.
 *
 * @see elgg_get_entities @warning 1.7's find_metadata() didn't support limits and returned all metadata.
 ...
 *
 * @param array $options
 *        	An options array. {@link elgg_get_metadata()}
 ...
 * exactly the same.
 *
 * @param string $e_table
 *        	Entities table name
 * @param string $n_table
 *        	Normalized metastrings table name (Where entities,
 ...
 *        	Array of names / values / operands
 * @param string $pair_operator
 *        	("AND" or "OR") Operator to use to join the where clauses for pairs
 * @param bool $case_sensitive
 *        	Case sensitive metadata names?
 * @param array|null $order_by_metadata
 *        	Array of names / direction
 * @param array|null $owner_guids
 *        	Array of owner GUIDs
 *        	
...
function _elgg_get_entity_metadata_where_sql($e_table, $n_table, $names = null, $values = null, $pairs = null, $pair_operator = 'AND', $case_sensitive = true, $order_by_metadata = null, $owner_guids = null) {
	return _elgg_services ()->metadataTable->getEntityMetadataWhereSql ( $e_table, $n_table, $names, $values, $pairs, $pair_operator, $case_sensitive, $order_by_metadata, $owner_guids );
}
 ...
 *
 * @param string $type
 *        	The type - object, user, etc
 * @param string $subtype
 *        	The subtype; all subtypes by default
 *        	
 * @return void
 */
