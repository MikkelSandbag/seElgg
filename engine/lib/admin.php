 ...
 *
 * @param array $options
 *        	Options array, @see elgg_get_entities() for parameters
 *        	
 ...
 *
 * @param string $id
 *        	A unique ID that your plugin can remember
 ...
 *
 * @param string $id
 *        	The unique ID assigned in add_admin_notice()
 *        	
 * @return bool
 * @since 1.8.0
 */
 ...
 *
 * @param string $id
 *        	The unique ID used to register the notice.
 *        	
 * @return bool
 * @since 1.8.0
 */
 ...
 *        	The menu section to add to
 * @param string $menu_id
 *        	The unique ID of section
 * @param string $parent_id
 *        	If a child section, the parent section id
 * @param int $priority
...
function _elgg_admin_init() {
	...
	foreach ( $widgets as $widget ) {
		elgg_register_widget_type ( $widget, elgg_echo ( "admin:widget:$widget" ), elgg_echo ( "admin:widget:$widget:help" ), array (
				'admin' 
		) );
	}
...
function _elgg_admin_pagesetup() {
	if (elgg_in_context ( 'admin' )) {
		...
		elgg_register_menu_item ( 'admin_header', array (
				'name' => 'admin_profile',
				'href' => false,
				'text' => elgg_echo ( 'admin:loggedin', array (
						$admin->name 
				) ),
				'priority' => 800 
		) );
...
function _elgg_robots_page_handler() {
	...
	$plugin_content = elgg_trigger_plugin_hook ( 'robots.txt', 'site', array (
			'site' => $site 
	), '' );
	if ($plugin_content) {
		$content = $content . "\n\n" . $plugin_content;
