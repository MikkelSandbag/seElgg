 ...
 *        	The view type, e.g. 'rss', or 'default'.
 *        	
 * @return bool
 */
 ...
 *
 * @param string $viewtype
 *        	Potential viewtype name. Alphanumeric chars plus _ allowed.
 *        	
 * @return bool
 * @access private
 * @since 1.9
 ...
 *        	Whether this view can be cached.
 * @return void
 * @since 1.9.0
 */
 ...
 *        	The view name
 * @param string $viewtype
 *        	If set, forces the viewtype
 * @param bool $recurse
 *        	If false, do not check extensions
 *        	
 * @return bool
 */
 ...
 *
 * @param string $view
 *        	The name and location of the view to use
 * @param array $vars
 *        	Variables to pass to the view.
 * @param boolean $bypass
 *        	If set to true, elgg_view will bypass any specified
 *        	alternative template handler; by default, it will
 *        	hand off to this if requested (see set_template_handler)
 * @param boolean $ignored
 *        	This argument is ignored and will be removed eventually
 * @param string $viewtype
 *        	If set, forces the viewtype for the elgg_view call to be
 ...
 *        	Suggestion with the deprecation message
 * @param string $version
 *        	Human-readable *release* version: 1.7, 1.8, ...
 *        	
 * @return string The parsed view
 * @access private
 */
 ...
 *        	This view is added to $view
 * @param int $priority
 *        	The priority, from 0 to 1000,
 *        	to add at (lowest numbers displayed first)
 * @param string $viewtype
 *        	I'm not sure why this is here @todo
 *        	
 * @return void
 * @since 1.7.0
 */
 ...
 *
 * @param string $view
 *        	The view that was extended.
 * @param string $view_extension
 *        	This view that was added to $view
 *        	
 * @return bool
 * @since 1.7.2
 */
 ...
 * @param string $body
 *        	Body
 * @param string $page_shell
 *        	Optional page shell to use. See page/shells view directory
 * @param array $vars
 *        	Optional vars array to pass to the page
 ...
 *        	The name of the view in page/layouts/.
 * @param array $vars
 *        	Associative array of parameters for the layout view
 *        	
 ...
 * @param array $vars
 *        	An associative array of display options for the menu.
 *        	Options include:
 ...
 *        	The menu item
 * @param array $vars
 *        	Options to pass to output/url if a link
 * @return string
 * @since 1.9.0
 */
function elgg_view_menu_item(\ElggMenuItem $item, array $vars = array()) {
	...
	if ($item->getHref () === false || $item->getHref () === null) {
		...
		if (preg_match ( '~<[a-z]~', $text )) {
			...
			return elgg_format_element ( 'span', array (
					'class' => 'elgg-non-link' 
			), $text );
		}
	}
 ...
 * @param array $vars
 *        	Array of variables to pass to the entity view.
 *        	In Elgg 1.7 and earlier it was the boolean $full_view
 * @param boolean $bypass
 *        	If true, will not pass to a custom template handler.
 *        	{@link set_template_handler()}
 * @param boolean $debug
 *        	Complain if views are missing
 *        	
 ...
 *        	The entity to display
 * @param string $size
 *        	The size: tiny, small, medium, large
 * @param array $vars
 *        	An array of variables to pass to the view. Some possible
 ...
 * @param array $vars
 *        	Variable array for view.
 * @param bool $bypass
 *        	If true, will not pass to a custom
 ...
 * @access private
 */
function elgg_view_entity_list($entities, $vars = array(), $offset = 0, $limit = null, $full_view = true, $list_type_toggle = true, $pagination = true) {
	if (! is_int ( $offset )) {
...
function elgg_view_entity_annotations(\ElggEntity $entity, $full_view = true) {
	...
	$annotations = elgg_trigger_plugin_hook ( 'entity:annotate', $entity_type, array (
			'entity' => $entity,
			'full_view' => $full_view 
	) );
 ...
 *        	The page title
 * @param array $vars
 *        	View variables (was submenu be displayed? (deprecated))
 *        	
...
function elgg_view_friendly_time($time) {
	return elgg_view ( 'output/friendlytime', array (
			'time' => $time 
	) );
}
 ...
 *
 * @param \ElggEntity $entity
 *        	The entity to view comments of
 * @param bool $add_comment
 *        	Include a form to add comments?
 * @param array $vars
 *        	Variables to pass to comment view
 *        	
 ...
 *
 * @param string $image
 *        	The icon and other information
 * @param string $body
 *        	Description content
 * @param array $vars
 ...
 *
 * @param string $type
 *        	The type of module (main, info, popup, aside, etc.)
 * @param string $title
 *        	A title to put in the header
 * @param string $body
 ...
 *
 * @param \ElggRiverItem $item
 *        	A river item object
 * @param array $vars
 *        	An array of variables for the view
 *        	
 ...
 *
 * @example <code>echo elgg_view_form('login');</code>
 *         
 ...
 *         
 * @param string $action
 *        	The name of the action. An action name does not include
 *        	the leading "action/". For example, "login" is an action name.
 * @param array $form_vars
 *        	$vars environment passed to the "input/form" view
 * @param array $body_vars
 *        	$vars environment passed to the "forms/$action" view
 *        	
 ...
 *
 * @param array $options
 *        	Any elgg_get_tags() options except:
 *        	
 ...
 * @param array $vars
 *        	Additional parameters for the rendering
 *        	
 * @return string
 * @since 1.8.0
 * @access private
 ...
 * @param string $class
 *        	Additional class: float, float-alt, or custom class
 *        	
...
function elgg_view_icon($name, $class = '') {
	...
	return elgg_view ( "output/icon", array (
			"class" => $icon_class 
	) );
}
...
function elgg_view_access_collections($owner_guid) {
	if ($collections = get_user_access_collections ( $owner_guid )) {
		...
		if ($user) {
			$entities = $user->getFriends ( array (
					'limit' => 0 
			) );
		} else {
			$entities = array ();
	...
	return elgg_view ( 'core/friends/collections', array (
 ...
 *        	The name of the function to pass to.
 *        	
 * @return bool
 * @see elgg_view()
 */
 ...
 *        	Optional The base of the view name without the view type.
 * @param string $folder
 *        	Required The folder to begin looking in
 * @param string $base_location_path
 *        	The base views directory to use with elgg_set_view_location()
 * @param string $viewtype
 *        	The type of view we're looking at (default, rss, etc)
 *        	
 ...
 * This ensures simplecache is cleared during upgrades.
 * See #2252
 *
 * @return void
 * @access private
