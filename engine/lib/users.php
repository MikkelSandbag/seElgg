 ...
 *
 * @param string $hash
 *        	Hash of the persistent login password
 *        	
 * @return \ElggUser
 */
 ...
 *        	
 * @param int $limit
 *        	Limit (deprecated usage, use $options)
 * @param int $offset
 *        	Offset (deprecated usage, use $options)
 * @param bool $count
 *        	Count (deprecated usage, use $options)
 *        	
 * @return \ElggUser[]|int
 */
 ...
 *        	Text (which will then be converted into a hash and stored)
 *        	
 * @return bool
 */
 ...
 * @param int $user_guid
 *        	The user id
 * @param string $conf_code
 *        	Confirmation code as sent in the request email.
 * @param string $password
 *        	Optional new password, if not randomly generated.
 *        	
...
function validate_username($username) {
	...
	if (strlen ( $username ) < $CONFIG->minusername) {
		$msg = elgg_echo ( 'registration:usernametooshort', array (
				$CONFIG->minusername 
		) );
	...
	if (strlen ( $username ) > 128) {
		$msg = elgg_echo ( 'registration:usernametoolong', array (
				128 
		) );
		throw new \RegistrationException ( $msg );
	}
...
']/u';
	...
	$blacklist2 = elgg_trigger_plugin_hook ( 'username:character_blacklist', 'user', array (
			'blacklist' => $blacklist2 
	), $blacklist2 );
	
	for($n = 0; $n < strlen ( $blacklist2 ); $n ++) {
		if (strpos ( $username, $blacklist2 [$n] ) !== false) {
			$msg = elgg_echo ( 'registration:invalidchars', array (
					$blacklist2 [$n],
					$blacklist2 
			) );
	...
	$result = true;
	return elgg_trigger_plugin_hook ( 'registeruser:validate:username', 'all', array (
			'username' => $username 
	), $result );
}
...
function validate_password($password) {
	...
	if (strlen ( $password ) < $CONFIG->min_password_length) {
		$msg = elgg_echo ( 'registration:passwordtooshort', array (
	...
	$result = true;
	return elgg_trigger_plugin_hook ( 'registeruser:validate:password', 'all', array (
			'password' => $password 
	), $result );
}
...
function validate_email_address($address) {
	...
	$result = true;
	return elgg_trigger_plugin_hook ( 'registeruser:validate:email', 'all', array (
			'email' => $address 
	), $result );
}
 ...
 * @param bool $allow_multiple_emails
 *        	Allow the same email address to be
 ...
 *
 * @param string $username
 *        	The username of the user sending the invitation
 *        	
 ...
 *        	The user's GUID
 * @param bool $status
 *        	Validated (true) or unvalidated (false)
 * @param string $method
...
function elgg_profile_fields_setup() {
		...
		foreach ( $fieldlistarray as $listitem ) {
			if ($translation = elgg_get_config ( "admin_defined_profile_{$listitem}" )) {
				...
				add_translation ( get_current_language (), array (
						"profile:admin_defined_profile_{$listitem}" => $translation 
				) );
			}
		}
	}
	...
	foreach ( $CONFIG->profile_fields as $name => $type ) {
		if ($type == 'tags' || $type == 'location' || $type == 'tag') {
			...
			add_translation ( get_current_language (), array (
					"tag_names:$name" => elgg_echo ( "profile:$name" ) 
			) );
		}
	}
}
