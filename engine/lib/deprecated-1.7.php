 ...
 *        	Can 'gallery' view can be displayed (default: no)
 *        	
 ...
 *        	Array of extensions (with .!) to ignore
 * @param array $list
 *        	A list files to include in the return
 *        	
 * @return array
 */
function get_library_files($directory, $exceptions = array(), $list = array()) {
	...
	return elgg_get_file_list ( $directory, $exceptions, $list, array (
			'.php' 
	) );
}
 ...
 *        	Offset
 * @param bool $count
 *        	Return a count or an array of entities
 * @param int $site_guid
 *        	Site GUID
 ...
 *        	Limit
 * @param bool $fullview
 *        	Show entity full views
 * @param bool $listtypetoggle
 *        	Show list type toggle
 * @param bool $allowedtypes
 *        	A string of the allowed types
 *        	
 * @return string
 ...
 *        	Limit
 * @param bool $fullview
 *        	Display entity full views?
 * @param bool $listtypetoggle
 *        	Allow switching to gallery mode?
 ...
 *        	The partial or full name or description
 * @param int $limit
 *        	Limit of the search.
 * @param int $offset
 *        	Offset.
 ...
 *        	The order.
 * @param boolean $count
 *        	Whether to return the count of results or just the results.
 *        	
 * @return mixed
 * @deprecated 1.7
 */
...
function search_list_groups_by_name($hook, $user, $returnvalue, $tag) {
	...
	if (! get_input ( 'offset' ) && (empty ( $object ) || $object == 'group')) {
		if ($groups = search_for_group ( $tag, $threshold )) {
			...
			$return = elgg_view ( 'group/search/startblurb', array (
					'count' => $countgroups,
					'tag' => $tag 
			) );
 ...
 *        	Metadata names case sensitivity
 *        	
 ...
 * @param bool $count
 *        	Return a count instead of entities
 * @param bool $meta_array_operator
 *        	Operator for metadata values
 *        	
 * @return int|array A list of entities, or a count if $count is set to true
 */
 ...
 *
 * @param string $criteria
 *        	The partial or full name or username.
 * @param int $limit
 *        	Limit of the search.
 * @param int $offset
 *        	Offset.
...
function search_list_objects_by_name($hook, $user, $returnvalue, $tag) {
	...
	if (! get_input ( 'offset' ) && (empty ( $object ) || $object == 'user')) {
		if ($users = search_for_user ( $tag, $threshold )) {
			...
			$return = elgg_view ( 'user/search/startblurb', array (
					'count' => $countusers,
					'tag' => $tag 
			) );
			...
			$return .= elgg_view ( 'user/search/finishblurb', array (
					'count' => $countusers,
					'threshold' => $threshold,
					'tag' => $tag 
			) );
 ...
 *        	The GUID of the relationship owner
...
function delete_object_entity($guid) {
	system_message ( elgg_echo ( 'deprecatedfunction', array (
			'delete_user_entity' 
	) ) );
