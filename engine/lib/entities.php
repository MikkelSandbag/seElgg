 ...
 *
 * @param string $type
 *        	The type you're subtyping (site, user, object, or group)
 * @param string $subtype
 *        	The subtype
 * @param string $class
 *        	Optional class name for the object
 *        	
 * @return int
 * @see update_subtype()
 * @see remove_subtype()
 ...
 *        	Subtype
 * @param string $class
 *        	Class name to use when loading this entity
 *        	
 * @return bool
 */
 ...
 *
 * @param int $user_guid
 *        	The user guid, or 0 for logged in user.
 * @param int $container_guid
 *        	The container, or 0 for the current page owner.
 * @param string $type
 *        	The type of entity we want to create (default: 'all')
 * @param string $subtype
 *        	The subtype of the entity we want to create (default: 'all')
 *        	
 * @return bool
 */
function can_write_to_container($user_guid = 0, $container_guid = 0, $type = 'all', $subtype = 'all') {
	...
	return elgg_trigger_plugin_hook ( 'container_permissions_check', $type, array (
			'container' => $container,
			'user' => $user,
			'subtype' => $subtype 
	), $return );
 ...
 *
 * @param int $guid
 *        	The GUID of the object to extract
 *        	
 ...
 *
 * @param \stdClass $row
 *        	The row of the entry in the entities table.
 *        	
 ...
 *        	Array of types or null if none.
 * @param null|array $subtypes
 *        	Array of subtypes or null if none
 * @param null|array $pairs
 *        	Array of pairs of types and subtypes
 *        	
 * @return false|string
 * @since 1.7.0
 * @access private
 ...
 *
 * @param string $column
 *        	Column name the guids should be checked against. Usually
 *        	best to provide in table.column format.
 * @param null|array $guids
 *        	Array of GUIDs.
 *        	
 * @return false|string
 * @since 1.8.0
 ...
 *          
 * @param array $options
 *        	Any options from $getter options plus:
 ...
 *        	The entity getter function to use to fetch the entities.
 * @param callback $viewer
 *        	The function to use to view the entity list.
 *        	
 * @return string
 * @since 1.7
 ...
 *        	The type of entity
 * @param string $subtype
 *        	The subtype of entity
 * @param int $container_guid
 *        	The container GUID that the entities belong to
 * @param int $site_guid
 *        	The site GUID
...
function get_entity_dates($type = '', $subtype = '', $container_guid = 0, $site_guid = 0, $order_by = 'time_created') {
	return _elgg_services ()->entityTable->getDates ( $type, $subtype, $container_guid, $site_guid, $order_by );
}
 ...
 *
 * @param string $type
 *        	The type of entity (object, site, user, group) or blank for all
 *        	
 ...
 *
 * @param array $options
 *        	Any elgg_get_entity() options plus:
 *        	
...
function elgg_list_registered_entities(array $options = array()) {
	...
	if (! empty ( $options ['type_subtype_pairs'] )) {
		$count = elgg_get_entities ( array_merge ( array (
				'count' => true 
		), $options ) );
		if ($count > 0) {
 ...
 *
 * @param int $guid
 *        	Entity annotation|relationship action carried out on
 * @param int $posted
 ...
 *
 * @return void @elgg_event_handler init system
 * @access private
 */
