...
class Elgg_DatabaseTest extends PHPUnit_Framework_TestCase {
	...
	public function test_runSqlScript_withOneStatement($script) {
		$db = $this->getDbMock ();
		$db->expects ( $this->exactly ( 1 ) )->method ( 'updateData' )->with ( $this->matchesRegularExpression ( "/^INSERT INTO test_sometable \(`key`\)\sVALUES \('Value(?: -- not a comment)?'\)$/" ) );
	...
	public function testFingerprintingOfCallbacks() {
		...
		$prints [$db->fingerprintCallback ( array (
				'foo',
				'bar' 
		) )] = true;
		$uniques ++;
	...
	private function getFixture($filename) {
		return dirname ( __FILE__ ) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'test_files' . DIRECTORY_SEPARATOR . 'sql' . DIRECTORY_SEPARATOR . $filename;
	}
	...
	private function getDbMock() {
		return $this->getMock ( $this->dbClass, array (
				'updateData' 
		), array (
				new $this->configClass ( ( object ) array (
						'dbprefix' => 'test_' 
				) ),
