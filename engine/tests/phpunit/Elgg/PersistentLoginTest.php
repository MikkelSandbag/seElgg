...
class PersistentLoginTest extends \PHPUnit_Framework_TestCase {
	...
	function setUp() {
		...
		$this->dbMock->expects ( $this->any () )->method ( 'sanitizeString' )->will ( $this->returnCallback ( array (
				$this,
				'mock_sanitizeString' 
		) ) );
	...
	function testLoginSavesHashAndPutsTokenInCookieAndSession() {
		$this->dbMock->expects ( $this->once () )->method ( 'insertData' )->will ( $this->returnCallback ( array (
				$this,
				'mock_insertData' 
		) ) );
	...
	function testRemoveDeletesHashAndDeletesTokenFromCookieAndSession() {
		...
		$this->dbMock->expects ( $this->once () )->method ( 'deleteData' )->will ( $this->returnCallback ( array (
				$this,
				'mock_deleteData' 
		) ) );
	...
	function testGettingUserFromKnownHashReturnsUser() {
		$this->dbMock->expects ( $this->once () )->method ( 'getDataRow' )->will ( $this->returnCallback ( array (
				$this,
				'mock_getDataRow' 
		) ) );
	...
	function testGettingMissingUserFromKnownHashReturnsNull() {
		$this->dbMock->expects ( $this->once () )->method ( 'getDataRow' )->will ( $this->returnValue ( ( object ) array (
				'guid' => 234 
		) ) );
	...
	function testLegacyCookiesAreReplacedInDbCookieAndSession() {
		...
		$this->dbMock->expects ( $this->once () )->method ( 'deleteData' );
	...
	function getMockElggUser($guid) {
		$user = $this->getMockBuilder ( '\ElggUser' )->disableOriginalConstructor ()->getMock ();
		$user->expects ( $this->any () )->method ( '__get' )->with ( 'guid' )->will ( $this->returnValue ( ( int ) $guid ) );
		return $user;
	}
	...
	protected function getSvcWithCookie($cookie_token = '') {
		...
		$svc->_callable_get_user = array (
				$this,
				'mock_get_user' 
		);
		$svc->_callable_generateToken = array (
				$this,
				'mock_generateToken' 
		);
		$svc->_callable_elgg_set_cookie = array (
				$this,
				'mock_elgg_set_cookie' 
		);
		$svc->_callable_sleep = array (
				$this,
				'mock_sleep' 
		);
		return $svc;
