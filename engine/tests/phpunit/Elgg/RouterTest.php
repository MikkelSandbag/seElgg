...
class RouterTest extends \PHPUnit_Framework_TestCase {
	...
	function testCanRegisterFunctionsAsPageHandlers() {
		$registered = $this->router->registerPageHandler ( 'hello', array (
				$this,
				'hello_page_handler' 
		) );
	...
	function testCanUnregisterPageHandlers() {
		$this->router->registerPageHandler ( 'hello', array (
				$this,
				'hello_page_handler' 
		) );
	...
	/**
	 * 1.
	 * Register a page handler for `/foo`
	...
	function testRouteSupportsSettingHandlerInHookResultForBackwardsCompatibility() {
		$this->router->registerPageHandler ( 'foo', array (
				$this,
				'foo_page_handler' 
		) );
		$this->hooks->registerHandler ( 'route', 'bar', array (
				$this,
				'bar_route_handler' 
		) );
	...
	function testRouteOverridenFromHook() {
		...
		$this->hooks->registerHandler ( 'route', 'foo', array (
				$this,
				'bar_route_override' 
		) );
