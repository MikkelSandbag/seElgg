...
class WidgetsServiceTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRegisterTypeParametersControl() {
		$service = new \Elgg\WidgetsService ( array (
				$this,
				'elgg_set_config' 
		) );
	...
	public function testCanRegisterType() {
		...
		$this->assertTrue ( $service->registerType ( 'widget_type_con', 'Widget name2', 'Widget description2', array (
				'dashboard',
				'profile' 
		) ) );
		$this->assertTrue ( $service->registerType ( 'widget_type_mul', 'Widget name3', 'Widget description3', array (
				'all' 
		), true ) );
		$this->assertTrue ( $service->registerType ( 'widget_type_con_mul', 'Widget name4', 'Widget description4', array (
				'dashboard',
				'settings' 
		), true ) );
		// overwrite
		$this->assertTrue ( $service->registerType ( 'widget_type_con_mul', 'Widget name5', 'Widget description5', array (
				'dashboard',
				'settings' 
		), true ) );
	...
	public function testRegistrationParametersPreserveNameDescription($service) {
		$resps = array (
				...
				'widget_type_con' => array (
						'Widget name2',
						'Widget description2' 
				),
				'widget_type_mul' => array (
						'Widget name3',
						'Widget description3' 
				),
				'widget_type_con_mul' => array (
						'Widget name5',
						'Widget description5' 
				) 
		);
