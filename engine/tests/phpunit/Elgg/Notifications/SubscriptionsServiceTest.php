...
class SubscriptionsServiceTest extends \PHPUnit_Framework_TestCase {
	public function setUp() {
		...
		$object = $this->getMock ( '\ElggObject', array (
				'getContainerGUID' 
		), array (), '', false );
		...
		$this->event = $this->getMock ( '\Elgg\Notifications\Event', array (
				'getObject' 
		), array (), '', false );
		$this->event->expects ( $this->any () )->method ( 'getObject' )->will ( $this->returnValue ( $object ) );
		$this->db = $this->getMock ( '\Elgg\Database', array (
				'getData',
				'getTablePrefix',
				'sanitizeString' 
		), array (), '', false );
	...
	public function testQueryGenerationForRetrievingSubscriptionRelationships() {
		...
		$this->db->expects ( $this->once () )->method ( 'getData' )->with ( $this->equalTo ( $query ) )->will ( $this->returnValue ( array () ) );
		$service = new \Elgg\Notifications\SubscriptionsService ( $this->db );
	...
	public function testGetSubscriptionsWithProperInput() {
		...
		$queryResult = array (
				$this->createObjectFromArray ( array (
						'guid' => '22',
