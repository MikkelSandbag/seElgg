...
class NotificationsServiceTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRegisterEvent() {
		...
		$service->registerEvent ( 'foo', 'bar', array (
				'test' 
		) );
		$events ['foo'] ['bar'] = array (
				'create',
				'test' 
		);
		$this->assertEquals ( $events, $service->getEvents () );
	...
	public function testEnqueueEventHook() {
		...
		$mock = $this->getMock ( '\Elgg\PluginHooksService', array (
				'trigger' 
		) );
		$mock->expects ( $this->once () )->method ( 'trigger' )->with ( 'enqueue', 'notification', $params, true );
	...
	public function testProcessQueueThreeEvents() {
		$mock = $this->getMock ( '\Elgg\Notifications\SubscriptionsService', array (
				'getSubscriptions' 
		), array (), '', false );
		$mock->expects ( $this->exactly ( 3 ) )->method ( 'getSubscriptions' )->will ( $this->returnValue ( array () ) );
	...
	public function testProcessQueueTimesout() {
		...
		$mock->expects ( $this->exactly ( 0 ) )->method ( 'getSubscriptions' );
		$service = new \Elgg\Notifications\NotificationsService ( $mock, $this->queue, $this->hooks, $this->access );
