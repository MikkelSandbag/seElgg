...
class EntityPreloaderTest extends \PHPUnit_Framework_TestCase {
	...
	public function setup() {
		$this->obj = new EntityPreloader ( array (
				'foo',
				'bar' 
		) );
		$dependency = new PreloaderMock_20140623 ();
		$this->obj->_callable_cache_checker = array (
				$dependency,
				'isCached' 
		);
		$this->obj->_callable_entity_loader = array (
				$dependency,
				'load' 
		);
	...
	public function testOnlyLoadsIfNotCached() {
		...
		$this->mock->expects ( $this->once () )->method ( 'load' )->with ( array (
				'guids' => array (
						234,
						345 
				) 
