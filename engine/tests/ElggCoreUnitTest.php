...
class IdenticalEntityExpectation extends EqualExpectation {
	 ...
	 * True if it exactly matches the held value.
	 *
	 * @param mixed $compare
	 *        	Comparison value.
	 * @return boolean
	...
	public function testMessage($compare) {
		...
		if ($this->test ( $compare )) {
			...
			return "Identical entity expectation [" . $dumper->describeValue ( $this->getValue () ) . "] fails with [" . $dumper->describeValue ( $compare ) . "] " . $dumper->describeDifference ( $value2, $compare2, TYPE_MATTERS );
		}
	}
}
