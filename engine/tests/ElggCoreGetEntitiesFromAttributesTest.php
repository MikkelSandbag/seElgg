...
class ElggCoreGetEntitiesFromAttributesTest extends \ElggCoreGetEntitiesBaseTest {
	...
	public function testGetSqlWithSinglePairAndStringValue() {
		...
		$expected = array (
				'joins' => array (
						"JOIN {$CONFIG->dbprefix}objects_entity type_table ON e.guid = type_table.guid" 
				),
				'wheres' => array (
						"((type_table.title = 'foo'))" 
				) 
		);
	...
	public function testGetSqlWithTwoPairs() {
		...
		$expected = array (
				'joins' => array (
						"JOIN {$CONFIG->dbprefix}users_entity type_table ON e.guid = type_table.guid" 
				),
				'wheres' => array (
						"((type_table.username = 'user2') AND (type_table.email = 'test@example.org'))" 
				) 
		);
		$this->assertIdentical ( $expected, $result );
	}
