...
class ElggCoreMetadataCacheTest extends \ElggCoreUnitTest {
	...
	public function testBasicApi() {
		...
		$this->assertIdentical ( $cache->load ( $guid, $this->name ), array (
				$this->value,
				1 
		) );
	...
	public function testWritesAreCached() {
		...
		$this->assertIdentical ( $this->cache->load ( $this->guid1, $this->name ), array (
				$this->value,
				1 
		) );
	...
	public function testPopulateFromEntities() {
		...
		$this->cache->flush ();
		$this->cache->populateFromEntities ( array (
				$this->guid1,
				$this->guid2 
		) );
	...
	public function testFilterHeavyEntities() {
		...
		$this->obj2->setMetadata ( $this->name, array (
				$big_str,
				$big_str 
		) );
