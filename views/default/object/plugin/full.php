...
$author = '<span>' . elgg_echo ( 'admin:plugins:label:author' ) . '</span>: ' . elgg_view ( 'output/text', array (
...
$copyright = elgg_view ( 'output/text', array (
		'value' => $plugin->getManifest ()->getCopyright () 
) );
$license = elgg_view ( 'output/text', array (
...
?>
	...
	<div class="elgg-plugin-more hidden"
		...
		if ($screenshots_html) {
			?>
			<div>
			<ul><?php echo $screenshots_html; ?></ul>
		</div>
			<?php
		}
		...
		<?php
		$contributors = elgg_view ( 'object/plugin/elements/contributors', array (
				'plugin' => $plugin 
		) );
		...
		<?php
		echo elgg_view ( 'object/plugin/elements/dependencies', array (
				'plugin' => $plugin 
		) );
		?>
		</div>
