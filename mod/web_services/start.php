...
function ws_init() {
	...
	elgg_ws_expose_function ( "auth.gettoken", "auth_gettoken", array (
			'username' => array (
					'type' => 'string' 
			),
			'password' => array (
 ...
 *
 * @param string $method
 *        	The api name to expose - for example "myapi.dosomething"
 * @param string $function
 *        	Your function callback.
 * @param array $parameters
 *        	(optional) List of parameters in the same order as in
 ...
 *        	(optional) Define what http method must be used for
 *        	this function. Default: GET
 * @param bool $require_api_auth
 *        	(optional) (default is false) Does this method
 ...
 */
function elgg_ws_expose_function($method, $function, array $parameters = NULL, $description = "", $call_method = "GET", $require_api_auth = false, $require_user_auth = false) {
	global $API_METHODS;
	...
	if ($parameters != NULL) {
		if (! is_array ( $parameters )) {
			$msg = elgg_echo ( 'InvalidParameterException:APIParametersArrayStructure', array (
					$method 
			) );
			throw new InvalidParameterException ( $msg );
		}
	...
	switch ($call_method) {
		...
		case 'GET' :
			...
			break;
		default :
			$msg = elgg_echo ( 'InvalidParameterException:UnrecognisedHttpMethod', array (
					$call_method,
					$method 
			) );
...
function ws_rest_handler() {
	...
	// Output the result
	echo elgg_view_page ( $method, elgg_view ( "api/output", array (
			"result" => $result 
	) ) );
}
