...
function authenticate_method($method) {
	...
	if (! isset ( $API_METHODS [$method] )) {
		throw new APIException ( elgg_echo ( 'APIException:MethodCallNotImplemented', array (
				$method 
		) ) );
	}
...
function execute_method($method) {
	...
	// method must be exposed
	if (! isset ( $API_METHODS [$method] )) {
		$msg = elgg_echo ( 'APIException:MethodCallNotImplemented', array (
				$method 
		) );
	...
	if (! is_string ( $function ) || ! is_callable ( $function )) {
		$msg = elgg_echo ( 'APIException:FunctionDoesNotExist', array (
				$method 
		) );
		throw new APIException ( $msg );
	}
	...
	if (strcmp ( get_call_method (), $API_METHODS [$method] ["call_method"] ) != 0) {
		$msg = elgg_echo ( 'CallException:InvalidCallMethod', array (
				$method,
	...
	if ($result === false) {
		$msg = elgg_echo ( 'APIException:FunctionParseError', array (
				$function,
	...
	if ($result === NULL) {
		// If no value
		$msg = elgg_echo ( 'APIException:FunctionNoReturn', array (
				$function,
				$serialised_parameters 
		) );
		throw new APIException ( $msg );
...
function verify_parameters($method, $parameters) {
	...
	foreach ( $API_METHODS [$method] ['parameters'] as $key => $value ) {
		...
		if (! is_array ( $value ) || ! isset ( $value ['type'] )) {
			...
			$msg = elgg_echo ( 'APIException:InvalidParameter', array (
					$key,
					$method 
			) );
			throw new APIException ( $msg );
		...
		if ($value ['required'] && ! array_key_exists ( $key, $parameters )) {
			$msg = elgg_echo ( 'APIException:MissingParameterInMethod', array (
					$key,
					$method 
			) );
			throw new APIException ( $msg );
		...
		switch (strtolower ( $value ['type'] )) {
			...
			case 'array' :
				...
				if (! is_array ( $parameters [$key] )) {
					$msg = elgg_echo ( 'APIException:ParameterNotArray', array (
							$key 
					) );
					throw new APIException ( $msg );
				}
				...
				break;
			default :
				$msg = elgg_echo ( 'APIException:UnrecognisedTypeCast', array (
						$value ['type'],
						$key,
						$method 
				) );
...
function api_auth_hmac() {
	...
	if ($api_header->method == "POST") {
		...
		if (strcmp ( $api_header->posthash, $calculated_posthash ) != 0) {
			$msg = elgg_echo ( 'SecurityException:InvalidPostHash', array (
					$calculated_posthash,
					$api_header->posthash 
			) );
...
function map_api_hash($algo) {
	...
	throw new APIException ( elgg_echo ( 'APIException:AlgorithmNotSupported', array (
			$algo 
	) ) );
}
 ...
 *        	URLEncoded string representation of the get variable parameters,
 *        	eg "method=user&guid=2"
 * @param string $post_hash
 *        	Optional sha1 hash of the post data.
 *        	
 ...
 * Firstly it verifies that a HMAC signature
 ...
 * it is present and is valid.
 * The user gets logged in so with the current
...
function _php_api_exception_handler($exception) {
	...
	echo elgg_view_page ( $exception->getMessage (), elgg_view ( "api/output", array (
			"result" => $result 
	) ) );
}
