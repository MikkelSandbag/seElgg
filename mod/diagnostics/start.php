...
function diagnostics_basic_hook($hook, $entity_type, $returnvalue, $params) {
	...
	$returnvalue .= elgg_echo ( 'diagnostics:report:basic', array (
			$release,
			$version 
	) );
...
function diagnostics_sigs_hook($hook, $entity_type, $returnvalue, $params) {
	...
	$returnvalue .= elgg_echo ( 'diagnostics:report:md5', array (
			diagnostics_md5_dir ( $base_dir ) 
	) );
...
function diagnostics_phpinfo_hook($hook, $entity_type, $returnvalue, $params) {
	...
	$returnvalue .= elgg_echo ( 'diagnostics:report:php', array (
			print_r ( $phpinfo, true ) 
	) );
...
function diagnostics_globals_hook($hook, $entity_type, $returnvalue, $params) {
	...
	$returnvalue .= elgg_echo ( 'diagnostics:report:globals', array (
			$output 
	) );
