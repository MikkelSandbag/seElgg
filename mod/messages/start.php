 ...
 *
 * @param array $page
 *        	Array of URL components for routing
 * @return bool
 */
...
function messages_notifier() {
	if (elgg_is_logged_in ()) {
		...
		if ($num_messages != 0) {
			...
			$tooltip .= " (" . elgg_echo ( "messages:unreadcount", array (
					$num_messages 
			) ) . ")";
		}
 ...
 * @param string $body
 *        	The body of the mesage
 * @param int $recipient_guid
 *        	The GUID of the user to send to
 * @param int $sender_guid
 *        	Optionally, the GUID of the user to send from
 * @param int $original_msg_guid
 *        	The GUID of the message to reply from (default: none)
 * @param bool $notify
 *        	Send a notification (default: true)
 * @param bool $add_to_sent
 *        	If true (default), will add a message to the sender's 'sent' tray
 * @return bool
 */
 ...
 *
 * @param int $user_guid
 *        	GUID of user whose inbox we're counting (0 for logged in user)
 * @param int $limit
 *        	Number of unread messages to return (default from settings)
 * @param int $offset
 *        	Start at a defined offset (for listings)
 * @param bool $count
 *        	Switch between entities array or count mode
 *        	
