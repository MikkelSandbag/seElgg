...
function tidypics_init() {
	...
	// Register widgets
	elgg_register_widget_type ( 'album_view', elgg_echo ( "tidypics:widget:albums" ), elgg_echo ( "tidypics:widget:album_descr" ), array (
			'profile' 
	) );
	elgg_register_widget_type ( 'latest_photos', elgg_echo ( "tidypics:widget:latest" ), elgg_echo ( "tidypics:widget:latest_descr" ), array (
			'profile' 
	) );
	
	if (elgg_is_active_plugin ( 'widget_manager' )) {
		// add index widgets for Widget Manager plugin
		elgg_register_widget_type ( 'index_latest_photos', elgg_echo ( "tidypics:mostrecent" ), elgg_echo ( 'tidypics:mostrecent:description' ), array (
				'index' 
		) );
		elgg_register_widget_type ( 'index_latest_albums', elgg_echo ( "tidypics:albums_mostrecent" ), elgg_echo ( 'tidypics:albums_mostrecent:description' ), array (
				'index' 
		) );
		
		// add groups widgets for Widget Manager plugin
		elgg_register_widget_type ( 'groups_latest_photos', elgg_echo ( "tidypics:mostrecent" ), elgg_echo ( 'tidypics:mostrecent:description' ), array (
				'groups' 
		) );
		elgg_register_widget_type ( 'groups_latest_albums', elgg_echo ( "tidypics:albums_mostrecent" ), elgg_echo ( 'tidypics:albums_mostrecent:description' ), array (
				'groups' 
		) );
	...
	// notifications
	elgg_register_notification_event ( 'object', 'album', array (
			'album_first',
			'album_more' 
	) );
...
function tidypics_entity_menu_setup($hook, $type, $return, $params) {
	...
	if (elgg_instanceof ( $entity, 'object', 'image' )) {
		...
		if (elgg_get_plugin_setting ( 'view_count', 'tidypics' )) {
			...
			$text = elgg_echo ( 'tidypics:views', array (
					( int ) $view_info ['total'] 
			) );
			$options = array (
					'name' => 'views',
...
function tidypics_notify_message($hook, $type, $notification, $params) {
	...
	if (elgg_instanceof ( $entity, 'object', 'album' )) {
		...
		if ($type == 'notification:album_first:object:album') {
			$notification->subject = elgg_echo ( 'tidypics:notify:subject_newalbum', array (
					$entity->title 
			), $language );
			$notification->body = elgg_echo ( 'tidypics:notify:body_newalbum', array (
					$owner->name,
					$title,
					$entity->getURL () 
			), $language );
			$notification->summary = elgg_echo ( 'tidypics:notify:summary_newalbum', array (
					$entity->title 
			), $language );
			
			return $notification;
		} else {
			$notification->subject = elgg_echo ( 'tidypics:notify:subject_updatealbum', array (
					$entity->title 
			), $language );
			$notification->body = elgg_echo ( 'tidypics:notify:body_updatealbum', array (
					$owner->name,
					$title,
					$entity->getURL () 
			), $language );
			$notification->summary = elgg_echo ( 'tidypics:notify:summary_updatealbum', array (
					$entity->title 
			), $language );
