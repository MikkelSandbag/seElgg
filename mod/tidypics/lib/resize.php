 ...
 *
 * @param
 *        	ElggFile holds the image that was uploaded
 * @param
 *        	string folder to store thumbnail in
 * @param
 *        	string name of the thumbnail
 ...
 * @param
 *        	bool - watermark this image?
 * @param int $maxwidth
 *        	The maximum width of the resized image
 * @param int $maxheight
 *        	The maximum height of the resized image
 * @param TRUE|FALSE $square
 *        	If set to TRUE, will take the smallest of maxwidth and maxheight and use it to set the dimensions on all size; the image will be cropped.
...
function tp_gd_resize($input_name, $output_name, $watermark, $maxwidth, $maxheight, $square = FALSE, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0) {
	...
	imagefilledrectangle ( $newimage, 0, 0, $new_width, $new_height, imagecolorallocate ( $newimage, 255, 255, 255 ) );
	
	$rtn_code = imagecopyresampled ( $newimage, $oldimage, 0, 0, $widthoffset, $heightoffset, $new_width, $new_height, $region_width, $region_height );
	if (! $rtn_code) {
		return $rtn_code;
	}
...
function tp_create_imagick_thumbnails($file, $prefix, $filestorename) {
	...
	$rtn_code = tp_imagick_resize ( $file->getFilenameOnFilestore (), $thumbname, $image_sizes ['tiny_image_width'], $image_sizes ['tiny_image_height'], TRUE );
	if (! $rtn_code) {
		return FALSE;
	}
	$file->thumbnail = $prefix . "thumb" . $filestorename;
	...
	$thumb->setFilename ( $prefix . "smallthumb" . $filestorename );
	$thumbname = $thumb->getFilenameOnFilestore ();
	$rtn_code = tp_imagick_resize ( $file->getFilenameOnFilestore (), $thumbname, $image_sizes ['small_image_width'], $image_sizes ['small_image_height'], TRUE );
	if (! $rtn_code) {
		return FALSE;
	}
	$file->smallthumb = $prefix . "smallthumb" . $filestorename;
	...
	$thumb->setFilename ( $prefix . "largethumb" . $filestorename );
	$thumbname = $thumb->getFilenameOnFilestore ();
	$rtn_code = tp_imagick_resize ( $file->getFilenameOnFilestore (), $thumbname, $image_sizes ['large_image_width'], $image_sizes ['large_image_height'], FALSE );
	if (! $rtn_code) {
		return FALSE;
	}
	$file->largethumb = $prefix . "largethumb" . $filestorename;
 ...
 *
 * @param string $input_name
 *        	The name of the file input field on the submission form
 * @param string $output_name
 *        	The name of the file to be written
 * @param int $maxwidth
 *        	The maximum width of the resized image
