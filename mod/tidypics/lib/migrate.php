...
function tidypics_migrate_user_pics(ElggUser $user) {
	...
	$user_album_entities = elgg_get_entities_from_metadata ( array (
	...
	foreach ( $pics as $pic ) {
		...
		if ($pic->container_guid != $user->getGUID () and $group = get_entity ( $pic->container_guid ) and $group instanceof ElggGroup) {
			...
			$group_album_entities = elgg_get_entities ( array (
					'type' => 'object',
					'subtype' => 'album',
...
function tidypics_migrate_pic_from_files($pic, $album_guid) {
	...
	foreach ( $file_md_fields as $md_name ) {
		...
		if (! ($old_fp = fopen ( $user_fs_path . $old_file, 'r' ) and $new_fp = fopen ( $user_fs_path . $new_file, 'w' ))) {
