...
function blog_get_page_content_read($guid = NULL) {
	...
	$return ['content'] = elgg_view_entity ( $blog, array (
			'full_view' => true 
	) );
 ...
 *        	The GUID of the page owner or NULL for all blogs
 * @return array
 */
function blog_get_page_content_list($container_guid = NULL) {
	...
	if ($container_guid) {
		...
		$return ['title'] = elgg_echo ( 'blog:title:user_blogs', array (
				$container->name 
		) );
 ...
 *        	The GUID of the owner of this page
 * @param int $lower
 *        	Unix timestamp
 * @param int $upper
 *        	Unix timestamp
...
function blog_get_page_content_archive($owner_guid, $lower = 0, $upper = 0) {
	...
	$title = elgg_echo ( 'date:month:' . date ( 'm', $lower ), array (
			date ( 'Y', $lower ) 
	) );
 ...
 *        	'edit' or 'new'
 * @param int $guid
 *        	GUID of blog post or container
 * @param int $revision
 *        	Annotation id for revision to edit (optional)
 * @return array
 */
