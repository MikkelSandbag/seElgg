...
echo '<div>' . elgg_echo ( 'twitter_api:usersettings:description', array (
		$site_name 
) ) . '</div>';
...
if (! $access_key || ! $access_secret) {
	...
	echo '<div>' . elgg_echo ( 'twitter_api:usersettings:request', array (
			$request_link,
			$site_name 
	) ) . '</div>';
} else {
	...
	if ($user->email) {
		...
		echo '<div>' . elgg_echo ( 'twitter_api:usersettings:authorized', array (
				$site_name,
				$twitter_name 
		) ) . '</div>';
		echo '<div>' . sprintf ( elgg_echo ( 'twitter_api:usersettings:revoke' ), $url ) . '</div>';
	} else {
		echo elgg_echo ( 'twitter_api:usersettings:cannot_revoke', array (
				elgg_normalize_url ( 'twitter_api/interstitial' ) 
		) );
	}
}
