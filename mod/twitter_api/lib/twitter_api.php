...
function twitter_api_forward() {
	...
	} elseif (0 === strpos ( $ref, elgg_get_site_url () ) && 0 !== strpos ( $ref, elgg_get_site_url () . 'twitter_api/' )) {
...
function twitter_api_login() {
	...
	if ($users) {
		...
		if (! $user) {
			...
			system_message ( elgg_echo ( 'twitter_api:login:email', array (
					$site_name 
			) ) );
...
function twitter_api_authorize() {
	...
	elgg_trigger_plugin_hook ( 'authorize', 'twitter_api', array (
			'token' => $token 
	) );
