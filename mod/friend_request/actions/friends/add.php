...
if (empty ( $user ) || empty ( $friend )) {
	...
	if (check_entity_relationship ( $friend->getGUID (), "friend", $user->getGUID () )) {
		try {
			...
			system_message ( elgg_echo ( "friends:add:successful", array (
					$friend->name 
			) ) );
			
			forward ( REFERER );
		} catch ( Exception $e ) {
			register_error ( elgg_echo ( "friends:add:failure", array (
					$friend->name 
			) ) );
			$errors = true;
		}
		...
		if (remove_entity_relationship ( $friend->getGUID (), "friendrequest", $user->getGUID () )) {
			...
			system_message ( elgg_echo ( "friend_request:approve:successful", array (
					$friend->name 
			) ) );
			...
			register_error ( elgg_echo ( "friend_request:approve:fail", array (
					$friend->name 
			) ) );
		}
	} else {
		try {
			if (! add_entity_relationship ( $user->getGUID (), "friendrequest", $friend->getGUID () )) {
				$errors = true;
				register_error ( elgg_echo ( "friend_request:add:exists", array (
						$friend->name 
				) ) );
			}
...
if (! $errors) {
	system_message ( elgg_echo ( "friend_request:add:successful", array (
			$friend->name 
	) ) );
}
