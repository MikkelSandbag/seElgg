...
function file_init() {
	...
	elgg_register_notification_event ( 'object', 'file', array (
			'create' 
	) );
...
function file_prepare_notification($hook, $type, $notification, $params) {
	...
	$notification->subject = elgg_echo ( 'file:notify:subject', array (
			$entity->title 
	), $language );
	...
	$notification->summary = elgg_echo ( 'file:notify:summary', array (
			$entity->title 
	), $language );
 ...
 *        	The GUID of the container of the files
 * @param bool $friends
 *        	Whether we're looking at the container or the container's friends
 *        	
...
function file_get_type_cloud($container_guid = "", $friends = false) {
	...
	if ($friends && $container) {
		...
		$friend_entities = $container->getFriends ( array (
				'limit' => 0 
		) );
		if ($friend_entities) {
			$friend_guids = array ();
