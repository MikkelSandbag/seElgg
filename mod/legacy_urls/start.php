...
function legacy_urls_redirect($url) {
	...
	switch ($method) {
		case 'landing' :
			$content = elgg_view ( 'legacy_urls/message', array (
					'url' => $url 
			) );
 ...
 *        	Additional query parameters in associate array
 * @return string
 */
...
function legacy_urls_tag_handler($segments) {
	$tag = $segments [0];
	$url = legacy_urls_prepare_url ( 'search', array (
			'q' => $tag 
	) );
