...
function pages_prepare_notification($hook, $type, $notification, $params) {
	...
	$notification->subject = elgg_echo ( 'pages:notify:subject', array (
			$title 
	), $language );
	...
	$notification->summary = elgg_echo ( 'pages:notify:summary', array (
			$entity->title 
	), $language );
...
function pages_is_page($value) {
	return ($value instanceof ElggObject) && in_array ( $value->getSubtype (), array (
			'page',
			'page_top' 
	) );
}
