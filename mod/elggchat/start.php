...
function elggchat_session_cleanup($hook, $entity_type, $returnvalue, $params) {
	...
	$session_count = elgg_get_entities ( array (
			'type' => "object",
	...
	$sessions = elgg_get_entities ( array (
			'type' => "object",
			'subtype' => ELGGCHAT_SESSION_SUBTYPE,
			'limit' => $session_count 
	) );
...
function elggchat_logout_handler($event, $object_type, $object) {
	...
	if (! empty ( $object ) && $object instanceof ElggUser) {
		...
		if ($chat_sessions_count > 0) {
			$sessions = $object->getEntitiesFromRelationship ( array (
					'relationship' => ELGGCHAT_MEMBER,
					'inverse_relationship' => true 
			) );
			...
			foreach ( $sessions as $session ) {
				...
				$session->annotate ( ELGGCHAT_SYSTEM_MESSAGE, elgg_echo ( 'elggchat:action:leave', array (
						$object->name 
				) ), ACCESS_LOGGED_IN, $object->guid );
				$session->save ();
