...
if ($user = elgg_get_logged_in_user_entity ()) {
	...
	$chat_sessions_count = elgg_get_entities_from_relationship ( array (
	...
	if ($chat_sessions_count > 0) {
		...
		// Generate sessions
		$chat_sessions = $user->getEntitiesFromRelationship ( array (
				'relationship' => ELGGCHAT_MEMBER,
				'inverse_relationship' => true 
		) );
		if (! empty ( $chat_sessions )) {
			...
			foreach ( $chat_sessions as $session ) {
				...
				if (! (($session->owner_guid != $user->guid) && ($session->countAnnotations ( ELGGCHAT_MESSAGE ) == 0))) {
					...
					$members = $session->getEntitiesFromRelationship ( array (
							'relationship' => ELGGCHAT_MEMBER 
					) );
					if (is_array ( $members ) && count ( $members ) > 1) {
						...
						foreach ( $members as $member ) {
							if ($member->guid != $user->guid) {
								...
								$result ["sessions"] [$session->guid] ["members"] [] = elgg_view ( "elggchat/user", array (
										"chatuser" => $member 
								) );
							}
						}
					} else {
						$result ["sessions"] [$session->guid] ["name"] = elgg_echo ( "elggchat:session:name:default", array (
								$session->guid 
						) );
					}
					...
					if ($msg_count > 0) {
						...
						foreach ( $annotations as $msg ) {
							$result ["sessions"] [$session->guid] ["messages"] [$msg->id] = elgg_view ( "elggchat/message", array (
									"message" => $msg,
	...
	if ($friends_count > 0) {
		...
		foreach ( $friends as $friend ) {
			if ($time - $friend->last_action <= $inactive) {
				$result ["friends"] ["online"] [] = elgg_view ( "elggchat/user", array (
						"chatuser" => $friend 
				) );
			} else {
				$result ["friends"] ["offline"] [] = elgg_view ( "elggchat/user", array (
						"chatuser" => $friend 
				) );
			}
		}
