...
if ($full) {
	...
	$info = "<table>";
	$info .= "<tr><td class='elggchat-chatsession-text'><label>" . elgg_echo ( 'elggchat:session_details:guid', array (
			$session->getGUID () 
	) ) . "</label></td></tr>";
	...
	$info .= elgg_view ( 'input/form', array (
			'body' => $form_body,
			'action' => $action_url 
	) );
	$info .= "<td><tr>";
	...
	$members = elgg_get_entities_from_relationship ( array (
			...
			'relationship_guid' => $session->getGUID () 
	) );
	if ($members) {
		$info .= "<table>";
		foreach ( $members as $member ) {
			...
			$result .= "<td width='30px'>" . elgg_view ( 'output/img', array (
					'src' => elgg_format_url ( $member->getIconURL ( 'tiny' ) ),
					'class' => 'messageIcon' 
			) ) . "</td>";
	...
	$info = "<table>";
	$info .= "<tr><td class='elggchat-chatsession-text'><label>" . elgg_echo ( 'elggchat:session:guid', array (
			$session->getGUID () 
	) ) . "</label><td><tr>";
	$info .= "<tr>";
