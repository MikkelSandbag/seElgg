...
class ElggInspector {
	...
	public function getViews() {
		...
		foreach ( $coreViews as $view ) {
			$views [$view] = array (
					$CONFIG->viewpath . "default/" . $view . ".php" 
			);
		}
	...
	public function getWidgets() {
		...
		foreach ( _elgg_services ()->widgets->getAllTypes () as $handler => $handler_obj ) {
			$tree [$handler] = array (
					$handler_obj->name,
					implode ( ',', array_values ( $handler_obj->context ) ) 
			);
		}
	 ...
	 *
	 * @return s array [menu name] => array(item name => array(text, href, section, parent))
	 *        
	 */
	 ...
	 *
	 * @param $dir full
	 *        	path to directory to begin search
