 ...
 *
 * @param array $page
 *        	From the page_handler function
 * @return bool
 */
...
function thewire_prepare_notification($hook, $type, $notification, $params) {
	...
	$subject = elgg_echo ( 'thewire:notify:subject', array (
			$owner->name 
	), $language );
	if ($entity->reply) {
		...
		if ($parent) {
			...
			$body = elgg_echo ( 'thewire:notify:reply', array (
					$owner->name,
					$parent_owner->name 
			), $language );
		}
	} else {
		$body = elgg_echo ( 'thewire:notify:post', array (
				$owner->name 
		), $language );
	}
	$body .= "\n\n" . $descr . "\n\n";
	$body .= elgg_echo ( 'thewire:notify:footer', array (
			$entity->getURL () 
	), $language );
	
	$notification->subject = $subject;
	$notification->body = $body;
	$notification->summary = elgg_echo ( 'thewire:notify:summary', array (
			$descr 
	), $language );
 ...
 *        	Parent post guid (if any)
 * @param string $method
 *        	The method (default: 'site')
 ...
 *        	Subscriptions for a notification event
 * @param array $params
 *        	Parameters including the event
 * @return array
 */
function thewire_add_original_poster($hook, $type, $subscriptions, $params) {
	...
	if ($entity && elgg_instanceof ( $entity, 'object', 'thewire' )) {
		$parent = $entity->getEntitiesFromRelationship ( array (
				'relationship' => 'parent' 
		) );
		if ($parent) {
			$parent = $parent [0];
