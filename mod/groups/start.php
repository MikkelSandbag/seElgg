...
function groups_init() {
	...
	elgg_register_widget_type ( 'group_activity', elgg_echo ( 'groups:widget:group_activity:title' ), elgg_echo ( 'groups:widget:group_activity:description' ), array (
			'dashboard' 
	), true );
	...
	foreach ( $profile_defaults as $name => $type ) {
		if ($type == 'tags') {
			...
			add_translation ( get_current_language (), array (
					"tag_names:$name" => elgg_echo ( "groups:$name" ) 
			) );
		}
	}
}
...
/**
 * Configure the groups sidebar menu.
 * Triggered on page setup
 */
function groups_setup_sidebar_menus() {
	...
	if (elgg_in_context ( 'group_profile' )) {
		...
		if (elgg_is_logged_in () && $page_owner->canEdit () && ! $page_owner->isPublicMembership ()) {
			...
			if ($count) {
				$text = elgg_echo ( 'groups:membershiprequests:pending', array (
						$count 
				) );
			} else {
	...
	if (elgg_get_context () == 'groups' && ! elgg_instanceof ( $page_owner, 'group' )) {
		...
		if ($user) {
			...
			if (is_array ( $invitations ) && ! empty ( $invitations )) {
				...
				$text = elgg_echo ( 'groups:invitations:pending', array (
						$invitation_count 
				) );
			} else {
 ...
 *
 * @param array $page
 *        	Array of url segments for routing
 * @return bool
 */
		...
		if ($page_owner->canWriteToContainer ( $user_guid )) {
			if ($page_owner->getContentAccessMode () == ElggGroup::CONTENT_ACCESS_MODE_MEMBERS_ONLY) {
				...
				$returnvalue = array (
						...
						$page_owner->group_acl => elgg_echo ( 'groups:acl', array (
								$page_owner->name 
						) ) 
				);
			} else {
...
/**
 * The default access for members only content is this group only.
 * This makes
 ...
 * @param int $user_guid
 *        	The user's guid
 * @param bool $return_guids
 *        	Return guids rather than ElggGroup objects
 *        	
...
/**
 * Function to use on groups for access.
 * It will house private, loggedin, public,
...
function group_access_options($group) {
	$access_array = array (
			...
			$group->group_acl => elgg_echo ( 'groups:acl', array (
					$group->name 
			) ) 
	);
	return $access_array;
 ...
 * @param string $type
 *        	Hook type
 * @param Elgg\Notifications\Notification $notification
...
function discussion_prepare_notification($hook, $type, $notification, $params) {
	...
	$notification->subject = elgg_echo ( 'discussion:topic:notify:subject', array (
			$title 
	), $language );
	...
	$notification->summary = elgg_echo ( 'discussion:topic:notify:summary', array (
			$entity->title 
	), $language );
...
function discussion_prepare_reply_notification($hook, $type, $notification, $params) {
	...
	$notification->subject = elgg_echo ( 'discussion:reply:notify:subject', array (
			$topic->title 
	), $language );
	...
	$notification->summary = elgg_echo ( 'discussion:reply:notify:summary', array (
			$topic->title 
	), $language );
 ...
 *        	Array containing subscriptions in the form
 ...
 *
 * @param ElggComment $entity
 *        	the comment $entity
 * @param ELggUser $group_owner
 *        	user who owns the group $group_owner
 * @return boolean
 */
 ...
 * @param array $params
 *        	Array('entity' => ElggEntity, 'user' => ElggUser)
 ...
 * @param array $return        	
 * @param array $params
 *        	Array with container, user and subtype
