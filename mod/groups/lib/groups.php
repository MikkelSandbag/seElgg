...
function groups_handle_all_page() {
	...
	$filter = elgg_view ( 'groups/group_sort_menu', array (
...
function groups_search_page() {
	...
	$title = elgg_echo ( 'groups:search:title', array (
			$display_query 
	) );
...
function groups_handle_owned_page() {
	...
	if ($page_owner->guid == elgg_get_logged_in_user_guid ()) {
		...
		$title = elgg_echo ( 'groups:owned:user', array (
				$page_owner->name 
		) );
	}
...
function groups_handle_mine_page() {
	...
	if ($page_owner->guid == elgg_get_logged_in_user_guid ()) {
		...
		$title = elgg_echo ( 'groups:user', array (
				$page_owner->name 
		) );
	}
	elgg_push_breadcrumb ( $title );
...
function groups_handle_edit_page($page, $guid = 0) {
	...
	if ($page == 'add') {
		...
		if (elgg_instanceof ( $group, 'group' ) && $group->canEdit ()) {
			...
			elgg_push_breadcrumb ( $title );
			$content = elgg_view ( "groups/edit", array (
					'entity' => $group 
			) );
		} else {
...
function groups_handle_profile_page($guid) {
	...
	$content = elgg_view ( 'groups/profile/layout', array (
			'entity' => $group 
	) );
	$sidebar = '';
	...
	if (elgg_group_gatekeeper ( false )) {
		if (elgg_is_active_plugin ( 'search' )) {
			$sidebar .= elgg_view ( 'groups/sidebar/search', array (
					'entity' => $group 
			) );
		}
		$sidebar .= elgg_view ( 'groups/sidebar/members', array (
				'entity' => $group 
		) );
...
function groups_handle_members_page($guid) {
	...
	$title = elgg_echo ( 'groups:members:title', array (
			$group->name 
	) );
	...
	$content = elgg_list_entities_from_relationship ( array (
			...
			'joins' => array (
					"JOIN {$db_prefix}users_entity u ON e.guid=u.guid" 
			),
			'order_by' => 'u.name ASC' 
	) );
 ...
 *
 * @param mixed $group
 *        	ElggGroup or null. If a group, uses values from the group.
 * @return array
 */
